...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    function openTrove(uint _maxFeePercentage, uint _debtTokenAmount, address _upperHint, address _lowerHint) external payable override {
        ...
        if (!isRecoveryMode) {
            vars.debtTokenFee = _triggerBorrowingFee(contractsCache.troveManager, contractsCache.debtToken, _debtTokenAmount, _maxFeePercentage);
        ...
        _withdrawDebtToken(contractsCache.activePool, contractsCache.debtToken, msg.sender, _debtTokenAmount, vars.netDebt);
        // Move the gas compensation to the Gas Pool
        _withdrawDebtToken(contractsCache.activePool, contractsCache.debtToken, gasPoolAddress, GAS_COMPENSATION, GAS_COMPENSATION);
    ...
    function moveFILGainToTrove(address _borrower, address _upperHint, address _lowerHint) external payable override {
    ...
    function withdrawDebtToken(uint _maxFeePercentage, uint _debtTokenAmount, address _upperHint, address _lowerHint) external override {
        _adjustTrove(msg.sender, 0, _debtTokenAmount, true, _upperHint, _lowerHint, _maxFeePercentage);
    }

    // Repay debt tokens to a Trove: Burn the repaid debt tokens, and reduce the trove's debt accordingly
    function repayDebtToken(uint _debtTokenAmount, address _upperHint, address _lowerHint) external override {
        _adjustTrove(msg.sender, 0, _debtTokenAmount, false, _upperHint, _lowerHint, 0);
    }

    function adjustTrove(uint _maxFeePercentage, uint _collWithdrawal, uint _debtTokenChange, bool _isDebtIncrease, address _upperHint, address _lowerHint) external payable override {
        _adjustTrove(msg.sender, _collWithdrawal, _debtTokenChange, _isDebtIncrease, _upperHint, _lowerHint, _maxFeePercentage);
    }
    ...
    */
    function _adjustTrove(address _borrower, uint _collWithdrawal, uint _debtTokenChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, debtToken);
        ...
        assert(msg.sender == _borrower || (msg.sender == stabilityPoolAddress && msg.value > 0 && _debtTokenChange == 0));
        ...
        if (_isDebtIncrease && !isRecoveryMode) { 
            vars.debtTokenFee = _triggerBorrowingFee(contractsCache.troveManager, contractsCache.debtToken, _debtTokenChange, _maxFeePercentage);
    ...
    function _triggerBorrowingFee(ITroveManager _troveManager, IDebtToken _debtToken, uint _debtTokenAmount, uint _maxFeePercentage) internal returns (uint) {
    ...
    {
        if (_isDebtIncrease) {
            _withdrawDebtToken(_activePool, _debtToken, _borrower, _debtTokenChange, _netDebtChange);
        } else {
    ...
    function _withdrawDebtToken(IActivePool _activePool, IDebtToken _debtToken, address _account, uint _debtTokenAmount, uint _netDebtIncrease) internal {
    ...
    function _repayDebtToken(IActivePool _activePool, IDebtToken _debtToken, address _account, uint _debtTokenAmount) internal {
    ...
    function _requireNonZeroAdjustment(uint _collWithdrawal, uint _debtTokenChange) internal view {
        require(msg.value != 0 || _collWithdrawal != 0 || _debtTokenChange != 0, "BorrowerOps: There must be either a collateral change or a debt change");
    }
    ...
    function _requireValidDebtRepayment(uint _currentDebt, uint _debtRepayment) internal pure {
        require(_debtRepayment <= _currentDebt.sub(GAS_COMPENSATION), "BorrowerOps: Amount repaid must not be larger than the Trove's debt");
    }
    ...
    function _requireCallerIsStabilityPool() internal view {
     ...
     function _requireSufficientDebtTokenBalance(IDebtToken _debtToken, address _borrower, uint _debtRepayment) internal view {
        require(_debtToken.balanceOf(_borrower) >= _debtRepayment, "BorrowerOps: Caller doesnt have enough tokens to make repayment");
    }
