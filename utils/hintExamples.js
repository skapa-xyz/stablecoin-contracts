...
async function main() {
 ...
 const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } = coreContracts
  ...
  const debtTokenAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 DebtToken
  const FILColl = toBN(toWei('5')) // borrower wants to lock 5 FIL collateral

  // Call deployed TroveManager contract to read the liquidation reserve and latest borrowing fee
  const liquidationReserve = await troveManager.GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(debtTokenAmount)
  
  // Total debt of the new trove = DebtToken amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = debtTokenAmount.add(expectedFee).add(liquidationReserve)

  // Get the nominal NICR of the new trove
  const _1e20 = toBN(toWei('100'))
  let NICR = FILColl.mul(_1e20).div(expectedDebt)
  ...
  await borrowerOperations.openTrove(maxFee, debtTokenAmount, upperHint, lowerHint, { value: FILColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 FIL
  const debtTokenRepayment = toBN(toWei('230')) // borrower wants to repay 230 DebtToken

  // Get trove's current debt and coll
  const {0: debt, 1: coll} = await troveManager.getEntireDebtAndColl(borrower)
  
  const newDebt = debt.sub(debtTokenRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, debtTokenRepayment, false, upperHint, lowerHint, {value: collIncrease})


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(debtTokenAmount, price, 50)

  const {0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncateddebtTokenAmount} = redemptionhint
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated DebtToken amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence. 
  */
  await troveManager.redeemCollateral(truncateddebtTokenAmount,
    firstRedemptionHint,
