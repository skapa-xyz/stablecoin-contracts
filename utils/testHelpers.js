...
class TestHelper {
    ...
    for (const account of accounts) {
      ...
      if (FILChangeBN.gt(zero)) {
        tx = await contracts.borrowerOperations.adjustTrove(
          ...
          upperHint,
          lowerHint,
          { from: account, value: FILChangeBN },
        );
  ...
  static async withdrawDebtToken_allAccounts(accounts, contracts, amount) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.withdrawDebtToken(
        this._100pct,
        amount,
        upperHint,
        lowerHint,
  ...
  static async repayDebtToken_allAccounts(accounts, contracts, amount) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.repayDebtToken(amount, upperHint, lowerHint, {
        from: account,
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async repayDebtToken_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.repayDebtToken(
        randDebtTokenAmount,
        upperHint,
        lowerHint,
        {
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      ...
      const tx = await stabilityPool.provideToSP(randomDebtTokenAmount, this.ZERO_ADDRESS, {
        from: account,
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async withdrawFILGainToTrove_allAccounts(accounts, contracts) {
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.stabilityPool.withdrawFILGainToTrove(upperHint, lowerHint, {
        from: account,
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
