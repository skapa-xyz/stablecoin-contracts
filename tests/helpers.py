...
def get_debt_token_amount_from_net_debt(contracts, net_debt):
...
def logGlobalState(contracts):
    ...
    activePoolColl = contracts.activePool.getFIL()
    activePoolDebt = contracts.activePool.getDebt()
    defaultPoolColl = contracts.defaultPool.getFIL()
    ...
    SP_DebtToken = contracts.stabilityPool.getTotalDebtTokenDeposits().to("ether")
    SP_FIL = contracts.stabilityPool.getFIL().to("ether")
    print('SP DebtToken         ', SP_DebtToken)
    print('SP FIL          ', SP_FIL)
    price_ether_current = contracts.priceFeedTestnet.getPrice()
    FIL_price = price_ether_current.to("ether")
    print('FIL price       ', FIL_price)
    ...
    return [FIL_price, num_troves, total_coll, total_debt, TCR, recovery_mode, last_ICR, SP_DebtToken, SP_FIL]
