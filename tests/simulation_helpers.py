...
GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for DebtToken. The ether price $P_t^e$ follows 
...
"""# ProtocolToken price
In the first month, the price of ProtocolToken follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_protocol_token) represents ProtocolToken price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_protocol_token, so that the expected ProtocolToken price increases from price_protocol_token_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_protocol_token_initial$ \cdot (1+$ drift_protocol_token$)^{720}$

The ProtocolToken price from the second month on is endogenously determined.
"""

#ProtocolToken price & airdrop
price_protocol_token_initial = 0.4
price_protocol_token = [price_protocol_token_initial]
sd_protocol_token=0.005
drift_protocol_token = 0.0035
supply_protocol_token=[0]
protocol_token_total_supply=100000000

"""**ProtocolToken Endogenous Price**

The staked ProtocolToken pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{ProtocolToken Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ protocol_token_total_supply

Therefore, the ProtocolToken price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{ProtocolToken\_total\_supply}E_t$$

Interpretation: The denominator implies that with more ProtocolTokens issued, ProtocolToken price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop ProtocolToken gain.
...
> (collateral in ether, debt in DebtToken, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of DebtToken.

$N_t^o$ is rounded to an integer.

---

The amount of DebtToken tokens generated by a new trove is
...
#sensitivity to DebtToken price & issuance fee
alpha = 0.3
...
#close troves
sd_closetroves=0.5
#sensitivity to DebtToken price
beta = 0.2
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of ProtocolToken airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of DebtToken deposited in the Stability Pool (see below)
...
print(" - FIL period 1 -")
print(f"Min FIL price: {min(price_ether[1:period1])}")
print(f"Max FIL price: {max(price_ether[1:period1])}")
for i in range(period1, period2):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether2))
print(" - FIL period 2 -")
print(f"Min FIL price: {min(price_ether[period1:period2])}")
print(f"Max FIL price: {max(price_ether[period1:period2])}")
for i in range(period2, period3):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether3))
print(" - FIL period 3 -")
print(f"Min FIL price: {min(price_ether[period2:period3])}")
print(f"Max FIL price: {max(price_ether[period2:period3])}")
for i in range(period3, period4):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether4))
print(" - FIL period 4 -")
print(f"Min FIL price: {min(price_ether[period3:period4])}")
print(f"Max FIL price: {max(price_ether[period3:period4])}")
...
"""ProtocolToken Price - First Month"""

#ProtocolToken price
for i in range(1, month):
    random.seed(2+13*i)
    shock_protocol_token = random.normalvariate(0,sd_protocol_token)  
    price_protocol_token.append(price_protocol_token[i-1]*(1+shock_protocol_token)*(1+drift_protocol_token))
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalDebtTokenDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_protocol_token_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_debt_token, price_protocol_token_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalDebtTokenDeposits() / 1e18
    stability_pool_eth_previous = contracts.stabilityPool.getFIL() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalDebtTokenDeposits() / 1e18
    stability_pool_eth_current = contracts.stabilityPool.getFIL() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_debt_token
    airdrop_gain = price_protocol_token_current * quantity_protocol_token_airdrop(index)

    data['liquidation_gain'][index] = liquidation_gain
    data['airdrop_gain'][index] = airdrop_gain

    return_stability = calculate_stability_return(contracts, price_debt_token, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_debt_token, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalDebtTokenDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_debt_token * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_debt_token, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    elif price_debt_token >=1:
        number_closetroves = max(0, n_steady * (1+shock_closetroves))
    else:
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_debt_token)*n_troves
    ...
    for i in range(0, len(drops)):
        ...
        coll = amounts['coll']
        debt = amounts['debt']
        pending = get_debt_token_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt)
        if pending == 0:
...
def get_debt_token_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    debtTokenBalance = contracts.debtToken.balanceOf(account)
    if debt > debtTokenBalance:
        pending = debt - debtTokenBalance
        ...
        # try with active accounts, which are more likely to hold DebtToken
        ...
        if pending > 0:
            print(f"\n ***Error: not enough DebtToken to repay! {debt / 1e18} DebtToken for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                pending = get_debt_token_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, repay_amount)
                if pending == 0:
                    contracts.borrowerOperations.repayDebtToken(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawDebtToken(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_debt_token_adjust = issuance_debt_token_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_debt_token_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + GAS_COMPENSATION
    debt_token = get_debt_token_amount_from_net_debt(contracts, floatToWei(supply_trove))
    if isNewTCRAboveCCR(contracts, coll, True, debtChange, True, floatToWei(price_ether_current)):
        contracts.borrowerOperations.openTrove(MAX_FEE, debt_token, hints[0], hints[1],
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_debt_token, index):
    random.seed(2019*index)
    shock_opentroves = random.normalvariate(0,sd_opentroves)
    n_troves = len(active_accounts)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    coll_added = 0
    issuance_debt_token_open = 0

    if index <= 0:
        number_opentroves = initial_open
    elif price_debt_token <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_debt_token - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_debt_token_open = issuance_debt_token_open + rate_issuance * supply_trove
    ...
    return [coll_added, issuance_debt_token_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    ...
    if stability_pool > stability_pool_previous:
        ...
        current_deposit = contracts.stabilityPool.getCompoundedDebtTokenDeposit(accounts[0])
...
"""DebtToken Price, liquidity pool, and redemption

**Price Determination**

---
With the supply and demand of DebtToken tokens defined above, the price of DebtToken at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of DebtToken.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_debt_token, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_debt_token_current= price_debt_token * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_debt_token_current

""" **Stabilizers**

There are two stabilizers to attenuate DebtToken price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the DebtToken price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of DebtToken in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of DebtToken price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    debt_token_balance = contracts.debtToken.balanceOf(accounts[i])
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedDebtTokenAmount] = contracts.hintHelpers.getRedemptionHints(debt_token_balance, price_ether_current, 70)
    if truncatedDebtTokenAmount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"DebtToken Token:    {contracts.debtToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"DebtToken bal: {debt_token_balance / 1e18}")
        print(f"truncated: {truncatedDebtTokenAmount / 1e18}")
        ...
        print(f"amount: {truncatedDebtTokenAmount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_debt_token, index):

    stability_pool = contracts.stabilityPool.getTotalDebtTokenDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    issuance_debt_token_stabilizer = 0
    ...
    #Calculating Price
    price_debt_token_current = calculate_price(price_debt_token, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.troveManager.getRedemptionRateWithDecay() / 1e18

    #Stabilizer
    #Ceiling Arbitrageurs
    if price_debt_token_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_debt_token)**delta
        ...
        issuance_debt_token_stabilizer = rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention):
            price_debt_token_current = 1.1 + rate_issuance
            liquidity_pool = supply_wanted - stability_pool


    #Floor Arbitrageurs
    if price_debt_token_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_debt_token)**delta
        ...
        if supply_diff < redemption_ratio * liquidity_pool:
            ...
            price_debt_token_current = 1 - rate_redemption
        else:
    ...
    return [price_debt_token_current, redemption_pool, redemption_fee, issuance_debt_token_stabilizer]

"""# ProtocolToken Market"""

def protocol_token_market(index, data):
    #quantity_protocol_token = (protocol_token_total_supply/3)*(1-0.5**(index/period))
    np.random.seed(2+3*index)
    if index <= month:
        price_protocol_token_current = price_protocol_token[index-1]
        ...
        price_protocol_token_current = discount * PE_ratio * annualized_earning / protocol_token_total_supply

    #MC_protocol_token_current = price_protocol_token_current * quantity_protocol_token

    return [price_protocol_token_current, annualized_earning]
