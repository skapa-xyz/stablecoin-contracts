...
const DebtToken = '0x5f98805A4E8be255a32880FDeC7F6728C6568bA0';
const CHAINLINK_ETHUSD_PROXY = '0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419';

const tokens = [DebtToken, WETH];
const weights = [toBigNum(dec(4, 17)), toBigNum(dec(6, 17))];

const NAME = 'WETH/DebtToken Pool';
const SYMBOL = '60WETH-40DT';
...
async function main() {
  ...
  */
  ...
  const debt_token_balance = INITIAL_FUNDING.mul(weights[0]).div(toBigNum(dec(1, 18)));
  const initialBalances = [debt_token_balance, weth_balance];
  th.logBN('Initial Debt Token', debt_token_balance);
  ...
  const txApprove = await debtToken.approve(VAULT, debt_token_balance);
  const approveReceipt = await txApprove.wait();
  console.log('Approve debt token gas: ', approveReceipt.gasUsed.toString());
  ...
  console.log('Total gas: ', receipt1.gasUsed.add(depositWethReceipt.gasUsed).add(approveWethReceipt.gasUsed).add(approveReceipt.gasUsed).add(receipt2.gasUsed).toString());
}
