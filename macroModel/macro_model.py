...
price_protocol_token_initial = 1
...
quantity_protocol_token_airdrop = 500
...
#close troves
sd_closetroves=0.5
#sensitivity to debt token price
beta = 0.2
...
#sensitivity to debt token price & issuance fee
alpha = 0.3
...
def liquidate_troves(troves, index, data):
  ...
  price_debt_token_previous = data.loc[index-1,'Price_debt_token']
  price_protocol_token_previous = data.loc[index-1,'price_protocol_token']
  ...
  ether_liquidated = troves_liquidated['FIL_Quantity'].sum()
  n_liquidate = troves_liquidated.shape[0]
  troves = troves.reset_index(drop = True)

  liquidation_gain = ether_liquidated*price_ether_current - debt_liquidated*price_debt_token_previous
  airdrop_gain = price_protocol_token_previous * quantity_protocol_token_airdrop
  ...
  if index <= day:
   return_stability = initial_return*(1+shock_return)
    ...
    return_stability = min(0.5, 365*(data.loc[index-day:index, 'liquidation_gain'].sum()+data.loc[index-day:index, 'airdrop_gain'].sum())/(price_debt_token_previous*stability_pool_previous))
  else:
    return_stability = (365/30)*(data.loc[index-month:index, 'liquidation_gain'].sum()+data.loc[index-month:index, 'airdrop_gain'].sum())/(price_debt_token_previous*stability_pool_previous)
  
  return[troves, return_stability, debt_liquidated, ether_liquidated, liquidation_gain, airdrop_gain, n_liquidate]

"""Close Troves"""

def close_troves(troves, index2, price_debt_token_previous):
  ...
  elif price_debt_token_previous >=1:
    number_closetroves = max(0, n_steady * (1+shock_closetroves))
  else:
    number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_debt_token_previous)*n_troves
...
def adjust_troves(troves, index):
  ...
  #A part of the troves are adjusted by adjusting debt
    if p >= ratio:
      if check<-1:
        working_trove['Supply'] = working_trove['FIL_Price']*working_trove['FIL_Quantity']/working_trove['CR_initial']
      if check>2:
        supply_new = working_trove['FIL_Price']*working_trove['FIL_Quantity']/working_trove['CR_initial']
        issuance_debt_token_adjust = issuance_debt_token_adjust + rate_issuance * (supply_new - working_trove['Supply'])
  ...
  #Another part of the troves are adjusted by adjusting collaterals
    if p < ratio and (check < -1 or check > 2):
      working_trove['FIL_Quantity'] = working_trove['CR_initial']*working_trove['Supply']/working_trove['FIL_Price']
    
    troves.loc[i] = working_trove
  return[troves, issuance_debt_token_adjust]

"""Open Troves"""

def open_troves(troves, index1, price_debt_token_previous):
  ...
  elif price_debt_token_previous <=1 + rate_issuance:
    number_opentroves = max(0, n_steady * (1+shock_opentroves))
  else:
    number_opentroves = max(0, n_steady * (1+shock_opentroves)) + alpha*(price_debt_token_previous-rate_issuance-1)*n_troves
  ...
  for i in range(0, number_opentroves):
    ...
    new_row = {"FIL_Price": price_ether_current, "FIL_Quantity": quantity_ether, 
  ...
  return[troves, number_opentroves, issuance_debt_token_open]
...
"""Debt Token Price, liquidity pool, and redemption"""
...
#Calculating Price
  ...
  price_debt_token_previous = float(data['Price_debt_token'][index-1])
  price_debt_token_current= price_debt_token_previous*((supply-stability_pool)/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
...
#Stabilizer
  ...
  if price_debt_token_current > 1.1 + rate_issuance:
    ...
    supply_wanted=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1.1+rate_issuance)/price_debt_token_previous)**delta
    ...
    new_row = {"FIL_Price": price_ether_current, "FIL_Quantity": quantity_ether, "CR_initial": CR_ratio,
  ...
  if price_debt_token_current < 1 - rate_redemption:
    ...
    #supply_current = sum(troves['Supply'])
    supply_target=stability_pool+liquidity_pool_previous*(drift_liquidity+shock_liquidity)*((1-rate_redemption)/price_debt_token_previous)**delta
    ...
    if supply_diff < redemption_ratio * liquidity_pool:
      ...
      price_debt_token_current= price_debt_token_previous * (liquidity_pool/(liquidity_pool_previous*(drift_liquidity+shock_liquidity)))**(1/delta)
    ...
    troves['FIL_Quantity'][wk] = troves['FIL_Quantity'][wk] - residual/price_ether_current
    troves['CR_current'][wk] = price_ether_current * troves['FIL_Quantity'][wk] / troves['Supply'][wk]
  ...
  troves = troves.reset_index(drop=True)
  return[price_debt_token_current, liquidity_pool, troves, issuance_debt_token_stabilizer, redemption_fee, n_redempt, redemption_pool, n_open]
...
def protocol_token_market(index, data):
  quantity_protocol_token = (100000000/3)*(1-0.5**(index/period))
  ...
  return[price_protocol_token_current, annualized_earning, MC_protocol_token_current]

"""# Simulation Program"""

#Defining Initials
initials = {"Price_debt_token":[1.00], "Price_FIL":[price_ether_initial], "n_open":[initial_open], "n_close":[0], "n_liquidate": [0], "n_redempt":[0], 
            "n_troves":[initial_open], "stability":[0], "liquidity":[0], "redemption_pool":[0],
            "supply_debt_token":[0],  "return_stability":[initial_return], "airdrop_gain":[0], "liquidation_gain":[0],  "issuance_fee":[0], "redemption_fee":[0],
            "price_protocol_token":[price_protocol_token_initial], "MC_protocol_token":[0], "annualized_earning":[0]}
data = pd.DataFrame(initials)
troves= pd.DataFrame({"FIL_Price":[], "FIL_Quantity":[], "CR_initial":[], 
              "Supply":[], "Rational_inattention":[], "CR_current":[]})
result_open = open_troves(troves, 0, data['Price_debt_token'][0])
troves = result_open[0]
issuance_debt_token_open = result_open[2]
data.loc[0,'issuance_fee'] = issuance_debt_token_open * initials["Price_debt_token"][0]
data.loc[0,'supply_debt_token'] = troves["Supply"].sum()
...
#exogenous ether price input
  ...
  troves['FIL_Price'] = price_ether_current
  price_debt_token_previous = data.loc[index-1,'Price_debt_token']
  price_protocol_token_previous = data.loc[index-1,'price_protocol_token']
...
#close troves
  result_close = close_troves(troves, index, price_debt_token_previous)
...
#adjust troves
  ...
  issuance_debt_token_adjust = result_adjustment[1]

#open troves
  result_open = open_troves(troves, index, price_debt_token_previous)
  troves = result_open[0]
...
#Calculating Price, Liquidity Pool, and Redemption
  ...
  price_debt_token_current = result_price[0]
  liquidity_pool = result_price[1]
  troves = result_price[2]
  issuance_debt_token_stabilizer = result_price[3]
...
#Summary
  issuance_fee = price_debt_token_current * (issuance_debt_token_adjust + issuance_debt_token_open + issuance_debt_token_stabilizer)
  n_troves = troves.shape[0]
  supply_debt_token = troves['Supply'].sum()
  if index >= month:
    price_protocol_token.append(price_protocol_token_current)

  new_row = {"Price_debt_token":float(price_debt_token_current), "Price_FIL":float(price_ether_current), "n_open":float(n_open), "n_close":float(n_close), 
             "n_liquidate":float(n_liquidate), "n_redempt": float(n_redempt), "n_troves":float(n_troves),
              "stability":float(stability_pool), "liquidity":float(liquidity_pool), "redemption_pool":float(redemption_pool), "supply_debt_token":float(supply_debt_token),
             ...
             "annualized_earning":float(annualized_earning), "MC_protocol_token":float(MC_protocol_token_current), "price_protocol_token":float(price_protocol_token_current)
             }
  data = data.append(new_row, ignore_index=True)
  if price_debt_token_current < 0:
    break
...
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['Price_debt_token'], name="Debt Token Price"),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['Price_FIL'], name="FIL Price"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Price Dynamics of Debt Token and FIL"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="Debt Token Price", secondary_y=False)
fig.update_yaxes(title_text="FIL Price", secondary_y=True)
fig.show()
...
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['supply_debt_token'], name="Debt Token Supply"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of Trove Numbers and Debt Token Supply"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="Number of Troves", secondary_y=False)
fig.update_yaxes(title_text="Debt Token Supply", secondary_y=True)
fig.show()
...
fig = make_subplots(specs=[[{"secondary_y": True}]])
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['price_protocol_token'], name="ProtocolToken Price"),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data.index/720, y=data['MC_protocol_token'], name="ProtocolToken Market Cap"),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of the Price and Market Cap of ProtocolToken"
)
fig.update_xaxes(tick0=0, dtick=1, title_text="Month")
fig.update_yaxes(title_text="ProtocolToken Price", secondary_y=False)
fig.update_yaxes(title_text="ProtocolToken Market Cap", secondary_y=True)
fig.show()
...
initials = {"Price_debt_token":[1.00], "Price_FIL":[price_ether_initial], "n_open":[initial_open], "n_close":[0], "n_liquidate": [0], "n_redempt":[0], 
            "n_troves":[initial_open], "stability":[0], "liquidity":[0], "redemption_pool":[0],
            "supply_debt_token":[0],  "return_stability":[initial_return], "airdrop_gain":[0], "liquidation_gain":[0],  "issuance_fee":[0], "redemption_fee":[0],
            "price_protocol_token":[price_protocol_token_initial], "MC_protocol_token":[0], "annualized_earning":[0], "base_rate":[base_rate_initial]}
data2 = pd.DataFrame(initials)
troves2= pd.DataFrame({"FIL_Price":[], "FIL_Quantity":[], "CR_initial":[], 
              "Supply":[], "Rational_inattention":[], "CR_current":[]})
result_open = open_troves(troves2, 0, data2['Price_debt_token'][0])
troves2 = result_open[0]
issuance_debt_token_open = result_open[2]
data2.loc[0,'issuance_fee'] = issuance_debt_token_open * initials["Price_debt_token"][0]
data2.loc[0,'supply_debt_token'] = troves2["Supply"].sum()
...
#exogenous ether price input
  price_ether_current = price_ether[index]
  troves2['FIL_Price'] = price_ether_current
  price_debt_token_previous = data2.loc[index-1,'Price_debt_token']
  price_protocol_token_previous = data2.loc[index-1,'price_protocol_token']
...
#close troves
  result_close = close_troves(troves2, index, price_debt_token_previous)
...
#open troves
  result_open = open_troves(troves2, index, price_debt_token_previous)
  troves2 = result_open[0]
  n_open = result_open[1]  
  issuance_debt_token_open = result_open[2]
...
#ProtocolToken Market
  result_protocol_token = protocol_token_market(index, data2)
  price_protocol_token_current = result_protocol_token[0]
  annualized_earning = result_protocol_token[1]
  MC_protocol_token_current = result_protocol_token[2]
...
#Summary
  ...
  supply_debt_token = troves2['Supply'].sum()
  if index >= month:
    price_protocol_token.append(price_protocol_token_current)
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['Price_debt_token'], name="Debt Token Price New", line = dict(dash='dot')),
    secondary_y=False,
)
fig.update_layout(
    title_text="Price Dynamics of Debt Token and FIL"
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['supply_debt_token'], name="Debt Token Supply New", line = dict(dash='dot')),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of Trove Numbers and Debt Token Supply"
...
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['price_protocol_token'], name="ProtocolToken Price New", line = dict(dash='dot')),
    secondary_y=False,
)
fig.add_trace(
    go.Scatter(x=data2.index/720, y=data2['MC_protocol_token'], name="ProtocolToken Market Cap New", line = dict(dash='dot')),
    secondary_y=True,
)
fig.update_layout(
    title_text="Dynamics of the Price and Market Cap of ProtocolToken"
