...
contract('BorrowerOperations', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_FIL, L_Debt snapshots", async () => {
      ...
      const { collateral: aliceCollBefore, totalDebt: aliceDebtBefore } = await openTrove({ extraDebtTokenAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      const { collateral: bobCollBefore, totalDebt: bobDebtBefore } = await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      const alicePendingFILReward = await troveManager.getPendingFILReward(alice)
      const bobPendingFILReward = await troveManager.getPendingFILReward(bob)
      ...
      for (reward of [alicePendingFILReward, bobPendingFILReward, alicePendingDebtReward, bobPendingDebtReward]) {
    ...
    it("withdrawColl(): sends the correct amount of FIL to the user", async () => {
      ...
      const alice_FILBalance_Before = toBN(web3.utils.toBN(await web3.eth.getBalance(alice)))
      await borrowerOperations.withdrawColl(dec(1, 'ether'), alice, alice, { from: alice, gasPrice: 0 })

      const alice_FILBalance_After = toBN(web3.utils.toBN(await web3.eth.getBalance(alice)))
    ...
    it("withdrawColl(): applies pending rewards and updates user's L_FIL, L_Debt snapshots", async () => {
      ...
      const pendingCollReward_A = await troveManager.getPendingFILReward(alice)
      const pendingDebtReward_A = await troveManager.getPendingDebtReward(alice)
      const pendingCollReward_B = await troveManager.getPendingFILReward(bob)
    ...
    it("withdrawDebtToken(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.withdrawDebtToken(th._100pct, debtTokenWithdrawal, alice, alice, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("withdrawDebtToken(): decays a non-zero base rate", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })

      const A_DebtTokenBal = await debtToken.balanceOf(A)
      ...
      await borrowerOperations.withdrawDebtToken(th._100pct, dec(1, 18), A, A, { from: D })
      ...
      await borrowerOperations.withdrawDebtToken(th._100pct, dec(1, 18), A, A, { from: E })
    ...
    it("withdrawDebtToken(): reverts if max fee > 100%", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdrawDebtToken(dec(2, 18), dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawDebtToken('1000000000000000001', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })
    ...
    it("withdrawDebtToken(): reverts if max fee < 0.5% in Normal mode", async () => {
      ...
      await assertRevert(borrowerOperations.withdrawDebtToken(0, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawDebtToken(1, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawDebtToken('4999999999999999', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdrawDebtToken(): reverts if fee exceeds max fee percentage", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(180, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      ...
      await assertRevert(borrowerOperations.withdrawDebtToken(lessThan5pct, dec(3, 18), A, A, { from: A }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate() // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 1%
      await assertRevert(borrowerOperations.withdrawDebtToken(dec(1, 16), dec(1, 18), A, A, { from: B }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 3.754%
      await assertRevert(borrowerOperations.withdrawDebtToken(dec(3754, 13), dec(1, 18), A, A, { from: C }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 0.5%%
      await assertRevert(borrowerOperations.withdrawDebtToken(dec(5, 15), dec(1, 18), A, A, { from: D }), "Fee exceeded provided maximum")
    })
    ...
    it("withdrawDebtToken(): succeeds when fee is less than max fee percentage", async () => {
      ...
      const tx1 = await borrowerOperations.withdrawDebtToken(moreThan5pct, dec(1, 18), A, A, { from: A })
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawDebtToken(dec(5, 16), dec(1, 18), A, A, { from: B })
      ...
      const tx3 = await borrowerOperations.withdrawDebtToken(dec(1, 17), dec(1, 18), A, A, { from: C })
      ...
      const tx4 = await borrowerOperations.withdrawDebtToken(dec(37659, 13), dec(1, 18), A, A, { from: D })
      assert.isTrue(tx4.receipt.status)

      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.withdrawDebtToken(dec(1, 18), dec(1, 18), A, A, { from: E })
    ...
    it("withdrawDebtToken(): doesn't change base rate if it is already zero", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      ...
      // D withdraws debt token
      await borrowerOperations.withdrawDebtToken(th._100pct, dec(37, 18), A, A, { from: D })
      ...
      // E opens trove 
      await borrowerOperations.withdrawDebtToken(th._100pct, dec(12, 18), A, A, { from: E })
    ...
    it("withdrawDebtToken(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await borrowerOperations.withdrawDebtToken(th._100pct, dec(1, 18), C, C, { from: C })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("withdrawDebtToken(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const withdrawalTx = await borrowerOperations.withdrawDebtToken(th._100pct, toBN(dec(37, 18)), D, D, { from: D })

        const emittedFee = toBN(th.getFeeFromDebtTokenBorrowingEvent(withdrawalTx))
    ...
    it("withdrawDebtToken(): reverts when calling address does not have active trove", async () => {
      ...
      const txBob = await borrowerOperations.withdrawDebtToken(th._100pct, dec(100, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to withdraw debt token
      try {
        const txCarol = await borrowerOperations.withdrawDebtToken(th._100pct, dec(100, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawDebtToken(): reverts when requested withdrawal amount is zero debt token", async () => {
      ...
      const txBob = await borrowerOperations.withdrawDebtToken(th._100pct, 1, bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Alice attempts to withdraw 0 token
      try {
        const txAlice = await borrowerOperations.withdrawDebtToken(th._100pct, 0, alice, alice, { from: alice })
    ...
    it("withdrawDebtToken(): reverts when system is in Recovery Mode", async () => {
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdrawDebtToken(th._100pct, dec(100, 18), alice, alice, { from: alice })
      assert.isTrue(txAlice.receipt.status)
    ...
    it("withdrawDebtToken(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
      ...
      try {
        const txBob = await borrowerOperations.withdrawDebtToken(th._100pct, dec(1, 18), bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawDebtToken(): reverts if system is in Recovery Mode", async () => {
      ...
      try {
        const txData = await borrowerOperations.withdrawDebtToken(th._100pct, '200', alice, alice, { from: alice })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
        assert.include(err.message, 'revert')
      }
    ...
    it("withdrawDebtToken(): increases the Trove's debt by the correct amount", async () => {
      ...
      await borrowerOperations.withdrawDebtToken(th._100pct, await getNetBorrowingAmount(100), alice, alice, { from: alice })
    ...
    it("withdrawDebtToken(): increases debt in ActivePool by correct amount", async () => {
      ...
      await borrowerOperations.withdrawDebtToken(th._100pct, await getNetBorrowingAmount(dec(10000, 18)), alice, alice, { from: alice })

      // check after
      const activePool_debtToken_After = await activePool.getDebt()
      th.assertIsApproximatelyEqual(activePool_debtToken_After, activePool_debtToken_Before.add(toBN(dec(10000, 18))))
    })
    ...
    it("withdrawDebtToken(): increases user DebtToken balance by correct amount", async () => {
      ...
      await borrowerOperations.withdrawDebtToken(th._100pct, dec(10000, 18), alice, alice, { from: alice })
    ...
    it("repayDebtToken(): reverts when repayment would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.repayDebtToken(debtTokenRepayment, alice, alice, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("repayDebtToken(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      ...
      const repayTxA = await borrowerOperations.repayDebtToken(1, A, A, { from: A })
      assert.isTrue(repayTxA.receipt.status)

      await borrowerOperations.openTrove(th._100pct, dec(20, 25), B, B, { from: B, value: dec(100, 30) })

      const repayTxB = await borrowerOperations.repayDebtToken(dec(19, 25), B, B, { from: B })
    ...
    it("repayDebtToken(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      ...
      const repayTxAPromise = borrowerOperations.repayDebtToken(2, A, A, { from: A })
    ...
    it("adjustTrove(): Reverts if repaid amount is greater than current debt", async () => {
      const { totalDebt } = await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      GAS_COMPENSATION = await borrowerOperations.GAS_COMPENSATION()
      const repayAmount = totalDebt.sub(GAS_COMPENSATION).add(toBN(1))
      await openTrove({ extraDebtTokenAmount: repayAmount, ICR: toBN(dec(150, 16)), extraParams: { from: bob } })

      await debtToken.transfer(alice, repayAmount, { from: bob })
    ...
    it("repayDebtToken(): reverts when calling address does not have active trove", async () => {
      ...
      const txBob = await borrowerOperations.repayDebtToken(dec(10, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to repayDebtToken
      try {
        const txCarol = await borrowerOperations.repayDebtToken(dec(10, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("repayDebtToken(): reverts when attempted repayment is > the debt of the trove", async () => {
      ...
      try {
        const txAlice = await borrowerOperations.repayDebtToken(aliceDebt.add(toBN(dec(1, 18))), alice, alice, { from: alice })
        assert.isFalse(txAlice.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("repayDebtToken(): reduces the Trove's debt token debt by the correct amount", async () => {
      ...
      await borrowerOperations.repayDebtToken(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })  // Repays 1/10 her debt
    ...
    it("repayDebtToken(): decreases debt token debt in ActivePool by correct amount", async () => {
      ...
      // check after
      const activePool_debtToken_After = await activePool.getDebt()
      th.assertIsApproximatelyEqual(activePool_debtToken_After, activePool_debtToken_Before.sub(aliceDebtBefore.div(toBN(10))))
    })
    ...
    it('repayDebtToken(): can repay debt in Recovery Mode', async () => {
      ...
      const tx = await borrowerOperations.repayDebtToken(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })
    ...
    it("repayDebtToken(): Reverts if borrower has insufficient debt token balance to cover his debt repayment", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      ...
      const repayDebtTokenPromise_B = borrowerOperations.repayDebtToken(toBN(dec(6, 18)), B, B, { from: B })

      await assertRevert(repayDebtTokenPromise_B, "Caller doesnt have enough tokens to make repayment")
    })
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, debtTokenRepayment, false, alice, alice, { from: alice, value: collTopUp }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("adjustTrove(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("adjustTrove(): borrowing at non-zero base rate sends debt token fee to LQTY staking contract", async () => {
      ...
      // D adjusts trove
      await openTrove({ extraDebtTokenAmount: toBN(dec(37, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const emittedFee = toBN(th.getFeeFromDebtTokenBorrowingEvent(adjustmentTx))
        assert.isTrue(emittedFee.gt(toBN('0')))
    ...
    it("adjustTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      await borrowerOperations.adjustTrove(th._100pct, 0, debtTokenRequest_D, true, D, D, { from: D })
    ...
    it("adjustTrove(): Borrowing at zero base rate changes LQTY staking contract debt token fees-per-unit-staked", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: toBN(dec(100, 'ether')) } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("adjustTrove(): Borrowing at zero base rate sends total requested debt token to the user", async () => {
      ...
      const D_DebtTOkenBalBefore = await debtToken.balanceOf(D)
      // Check baseRate is zero
      const baseRate_1 = await troveManager.baseRate()
      assert.equal(baseRate_1, '0')
      ...
      const debtTokenBalanceAfter = await debtToken.balanceOf(D)
    ...
    it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const CCR = await troveManager.CCR()
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(200000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      // Check emitted fee = 0
      const emittedFee = toBN(await th.getEventArgByName(txAlice, 'DebtTokenBorrowingFeePaid', '_debtTokenFee'))
      ...
      assert.equal(lqtyStakingdebtTokenBalanceAfter.toString(), lqtyStakingDebtTokenBalanceBefore.toString())
    })
    ...
    it("adjustTrove(): reverts when debt token repaid is > debt of the trove", async () => {
      ...
      const remainingDebt = (await troveManager.getTroveDebt(bob)).sub(GAS_COMPENSATION)
    ...
    it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

      await priceFeed.setPrice(dec(100, 18))

      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: bob } })
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
    ...
    it("adjustTrove(): Changes the debt in ActivePool by requested increase", async () => {
      ...
      th.assertIsApproximatelyEqual(activePool_debt_After, activePool_debt_Before.add(toBN(dec(100, 18))))
    })
    ...
    it("adjustTrove(): Reverts if borrower has insufficient debt token balance to cover his debt repayment", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: B } })
      ...
      const B_DebtTokenBal = await debtToken.balanceOf(B)
      assert.isTrue(B_DebtTokenBal.lt(bobDebt))

      const repayDebtTokenPromise_B = borrowerOperations.adjustTrove(th._100pct, 0, bobDebt, false, B, B, { from: B })
    ...
    if (!withProxy) { // no need to test this with proxies
      it("Internal _adjustTrove(): reverts when op is a withdrawal and _borrower param is not the msg.sender", async () => {
        await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
        await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
    ...
    it("closeTrove(): reverts when it would lower the TCR below CCR", async () => {
      ...
      await openTrove({ ICR: toBN(dec(120, 16)), extraDebtTokenAmount: toBN(dec(300, 18)), extraParams:{ from: bob } })
    ...
    it("closeTrove(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(100000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      ...
      const aliceCollBefore = await getTroveEntireColl(alice)
      const dennisDebtToken = await debtToken.balanceOf(dennis)
      assert.isTrue(aliceCollBefore.gt(toBN('0')))
      assert.isTrue(dennisDebtToken.gt(toBN('0')))

      // To compensate borrowing fees
      await debtToken.transfer(alice, dennisDebtToken.div(toBN(2)), { from: dennis })
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_FIL_A_Snapshot = (await troveManager.rewardSnapshots(alice))[0]
      ...
      const L_FIL_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(alice))[0]
      ...
      const L_FIL_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(alice))[0]
    ...
    if (!withProxy) { // TODO: wrap web3.eth.getBalance to be able to go through proxies
      it("closeTrove(): sends the correct amount of FIL to the user", async () => {
        ...
        const alice_FILBalance_Before = web3.utils.toBN(await web3.eth.getBalance(alice))
        ...
        const alice_FILBalance_After = web3.utils.toBN(await web3.eth.getBalance(alice))
        const balanceDiff = alice_FILBalance_After.sub(alice_FILBalance_Before)
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's DebtToken balance", async () => {
      ...
      const alice_DebtTokenBalance_After = await debtToken.balanceOf(alice)
      th.assertIsApproximatelyEqual(alice_DebtTokenBalance_After, alice_DebtTokenBalance_Before.sub(aliceDebt.sub(GAS_COMPENSATION)))
    })
    ...
    it("closeTrove(): applies pending rewards", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      assert.isAtMost(th.getDifference(defaultPool_FIL_afterAliceCloses,
        defaultPool_FIL.sub(pendingCollReward_A)), 1000)
      assert.isAtMost(th.getDifference(defaultPool_Debt_afterAliceCloses,
        defaultPool_Debt.sub(pendingDebtReward_A)), 1000)
    ...
    it("closeTrove(): reverts if borrower has insufficient debt token balance to repay his entire debt", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      ...
      await assertRevert(closeTrovePromise_B, "BorrowerOps: Caller doesnt have enough tokens to make repayment")
    })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): emits a TroveUpdated event with the correct collateral and debt", async () => {
        const txA = (await openTrove({ extraDebtTokenAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })).tx
        const txB = (await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })).tx
        const txC = (await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })).tx
        ...
        const txD = (await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })).tx
        const txE = (await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })).tx
    ...
    it("openTrove(): decays a non-zero base rate", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // E opens trove 
      await openTrove({ extraDebtTokenAmount: toBN(dec(12, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("openTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        // D withdraws debt token
        const openTroveTx = await borrowerOperations.openTrove(th._100pct, D_DebtTokenRequest, ZERO_ADDRESS, ZERO_ADDRESS, { from: D, value: dec(200, 'ether') })

        const emittedFee = toBN(th.getFeeFromDebtTokenBorrowingEvent(openTroveTx))
        assert.isTrue(toBN(emittedFee).gt(toBN('0')))

        const newDebt = (await troveManager.Troves(D))[0]

        // Check debt on Trove struct equals drawn debt plus emitted fee
        th.assertIsApproximatelyEqual(newDebt, D_DebtTokenRequest.add(emittedFee).add(GAS_COMPENSATION), 100000)
      })
    }
    ...
    it("openTrove(): Borrowing at non-zero base rate sends requested amount to the user", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, debtTokenRequest_D, D, D, { from: D, value: dec(500, 'ether') })
    ...
    it("openTrove(): Borrowing at zero base rate changes the LQTY staking contract debt token fees-per-unit-staked", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("openTrove(): Borrowing at zero base rate charges minimum fee", async () => {
      ...
      const txC = await borrowerOperations.openTrove(th._100pct, debtTokenRequest, ZERO_ADDRESS, ZERO_ADDRESS, { value: dec(100, 'ether'), from: C })
      const _debtTokenFee = toBN(th.getEventArgByName(txC, "DebtTokenBorrowingFeePaid", "_debtTokenFee"))
    ...
    it("openTrove(): reverts when system is in Recovery Mode and ICR < CCR", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      assert.isFalse(await th.checkRecoveryMode(contracts))
      ...
      try {
        const txBob = await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: alice } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when trove ICR < MCR", async () => {
      ...
      try {
        const txBob = (await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })).tx
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
      ...
      try {
        const txBob = await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when opening the trove would cause the TCR of the system to fall below the CCR", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      ...
      try {
        const txBob = await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts if trove is already active", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: bob } })

      try {
        const txB_1 = await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: bob } })
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      const txCarol = (await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: carol } })).tx
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, MIN_NET_DEBT, carol, carol, { from: alice, value: dec(100, 'ether') })
    ...
    it("openTrove(): allows a user to open a Trove, then close it, then re-open it", async () => {
      // Open Troves
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("openTrove(): increases the Trove's debt by the correct amount", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(10000, 18)), alice, alice, { from: alice, value: dec(100, 'ether') })
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        await borrowerOperations.openTrove(th._100pct, troveDebtTokenAmount, alice, alice, { from: alice, value: troveColl })
        await borrowerOperations.openTrove(th._100pct, troveDebtTokenAmount, bob, bob, { from: bob, value: troveColl })
    ...
    if (!withProxy) {
      it('closeTrove(): fails if owner cannot receive FIL', async () => {
        ...
        await th.assertRevert(nonPayable.forward(borrowerOperations.address, closeTroveData), 'ActivePool: sending FIL failed')
      })
    }
  }
