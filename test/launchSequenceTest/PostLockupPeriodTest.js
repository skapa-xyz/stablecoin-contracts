...
contract("After the initial lockup period has passed", async (accounts) => {
  ...
  describe("Transferring ProtocolToken", async (accounts) => {
    ...
    it("Anyone can transfer to an EOA", async () => {
      // Start D, E, liquityAG with some ProtocolToken
      await protocolToken.unprotectedMint(D, dec(1, 24));
      await protocolToken.unprotectedMint(E, dec(2, 24));
      await protocolToken.unprotectedMint(liquityAG, dec(3, 24));
      ...
      const protocolTokenTransferTx_2 = await protocolToken.transfer(liquityAG, dec(1, 18), {
        from: E,
      });
    ...
    it("Anyone can approve any EOA to spend their ProtocolToken", async () => {
      ...
      const protocolTokenApproveTx_3 = await protocolToken.approve(liquityAG, dec(1, 18), {
        from: F,
      });
    ...
    it("Anyone can increaseAllowance for any EOA or Liquity contract", async () => {
      ...
      const protocolTokenIncreaseAllowanceTx_3 = await protocolToken.increaseAllowance(
        multisig,
        dec(1, 18),
        { from: F },
      );
      ...
      await assert.isTrue(protocolTokenIncreaseAllowanceTx_3.receipt.status);
      ...
      // Increase allowance of ProtocolToken contracts from F
    ...
    it("Anyone can decreaseAllowance for any EOA or Liquity contract", async () => {
      ...
      const protocolTokenApproveTx_2 = await protocolToken.approve(B, dec(1, 18), { from: G });
      const protocolTokenApproveTx_3 = await protocolToken.approve(multisig, dec(1, 18), {
        from: F,
      });
      await assert.isTrue(protocolTokenApproveTx_1.receipt.status);
      await assert.isTrue(protocolTokenApproveTx_2.receipt.status);
      ...
      const protocolTokenDecreaseAllowanceTx_3 = await protocolToken.decreaseAllowance(
        multisig,
        dec(1, 18),
        { from: F },
      );
      ...
      await assert.isTrue(protocolTokenDecreaseAllowanceTx_3.receipt.status);
      ...
      // Decrease allowance of ProtocolToken contracts from F
      for (const contract of Object.keys(protocolTokenContracts)) {
        const protocolTokenIncreaseAllowanceTx = await protocolToken.decreaseAllowance(
          protocolTokenContracts[contract].address,
          dec(1, 18),
          { from: F },
        );
      ...
      // Decrease allowance of ProtocolToken contracts from multisig
      for (const contract of Object.keys(protocolTokenContracts)) {
        const protocolTokenIncreaseAllowanceTx = await protocolToken.decreaseAllowance(
          protocolTokenContracts[contract].address,
          dec(1, 18),
