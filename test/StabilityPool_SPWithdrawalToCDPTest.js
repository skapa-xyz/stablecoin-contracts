...
contract("StabilityPool - Withdrawal of stability deposit - Reward calculations", async () => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawFILGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and FIL Gain after one liquidation", async () => {
      ...
      const txA = await stabilityPool
        .connect(alice)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txB = await stabilityPool
        .connect(bob)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txC = await stabilityPool
        .connect(carol)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedDebtTokenDeposit(alice.address)).toString(),
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedDebtTokenDeposit(carol.address)).toString(),
          "6666666666666666666666",
        ),
        10000,
      );
    ...
    it("withdrawFILGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedDebtTokenDeposit(erin.address)).toString(),
    ...
    it("withdrawFILGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const txE = await stabilityPool
        .connect(erin)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txF = await stabilityPool
        .connect(flyn)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txG = await stabilityPool
        .connect(graham)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txH = await stabilityPool
        .connect(harriet)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      ...
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedDebtTokenDeposit(flyn.address)).toString(),
          "0",
        ),
        100000,
      );
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedDebtTokenDeposit(graham.address)).toString(),
          "0",
        ),
        100000,
      );
      assert.isAtMost(
        th.getDifference(
          (await stabilityPool.getCompoundedDebtTokenDeposit(harriet.address)).toString(),
          "0",
        ),
        100000,
      );
    ...
    */
    it("withdrawFILGainToTrove(): Several deposits of 10000 DebtToken span one scale factor change. Depositors withdraws correct compounded deposit and FIL Gain after one liquidation", async () => {
      ...
      assert.equal(await stabilityPool.getCompoundedDebtTokenDeposit(alice.address), "0");
      assert.equal(await stabilityPool.getCompoundedDebtTokenDeposit(alice.address), "0");
      assert.equal(await stabilityPool.getCompoundedDebtTokenDeposit(alice.address), "0");
    ...
    it("withdrawFILGainToTrove(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18));
      const txA = await stabilityPool.connect(A).withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txB = await stabilityPool.connect(B).withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      await priceFeed.setPrice(dec(100, 18));
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18));
      const txC = await stabilityPool.connect(C).withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txD = await stabilityPool.connect(D).withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      await priceFeed.setPrice(dec(100, 18));
      ...
      // Attempt withdrawals
      const txE = await stabilityPool.connect(E).withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txF = await stabilityPool.connect(F).withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
    ...
    it("withdrawFILGainToTrove(): Large liquidated coll/debt, deposits and FIL price", async () => {
      ...
      const aliceDebtTokenBalance = await stabilityPool.getCompoundedDebtTokenDeposit(
        alice.address,
      );
      ...
      const bobDebtTokenBalance = await stabilityPool.getCompoundedDebtTokenDeposit(bob.address);
    ...
    it("withdrawFILGainToTrove(): Small liquidated coll/debt, large deposits and FIL price", async () => {
      ...
      const txAPromise = stabilityPool
        .connect(alice)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
      const txBPromise = stabilityPool
        .connect(bob)
        .withdrawFILGainToTrove(ZERO_ADDRESS, ZERO_ADDRESS);
