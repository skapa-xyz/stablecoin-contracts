...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100000, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100000, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100000, 18), extraParams: { from: C } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(20000, 16)), extraDebtTokenAmount: dec(10, 18), extraParams: { from: D } })
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100000, 18), extraParams: { from: E } })
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const D_expectedPendingFIL_2 = C_collRedistribution.mul(D_entireColl_1).div(denominatorColl_2)
      .add(D_expectedPendingFIL_1)

    const E_expectedPendingFIL_2 = C_collRedistribution.mul(E_entireColl_1).div(denominatorColl_2)
    ...
    const D_expectedPendingFIL_3 = E_collRedistribution.mul(D_entireColl_2).div(denominatorColl_3)
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(110, 18), extraParams: { from: carol } })
    ...
    await borrowerOperations.withdrawDebtToken(th._100pct, await getNetBorrowingAmount(A_totalDebt), alice, alice, { from: alice })
    ...
    assert.isAtMost(th.getDifference(bob_debt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)), 1000)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    // D opens trove
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(110, 18), extraParams: { from: dennis } })
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // A, B, C, D open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ extraDebtTokenAmount: dec(110, 18), extraParams: { from: carol, value: _998_Ether } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(110, 18), extraParams: { from: dennis, value: dec(1000, 'ether') } })
    ...
    const entireSystemColl_1 = (await activePool.getFIL()).add(await defaultPool.getFIL()).toString()
    ...
    th.assertIsApproximatelyEqual(alice_FILReward_1.toString(), th.applyLiquidationFee(D_coll).mul(A_coll).div(totalColl))
    th.assertIsApproximatelyEqual(bob_FILReward_1.toString(), th.applyLiquidationFee(D_coll).mul(B_coll).div(totalColl))
    th.assertIsApproximatelyEqual(carol_FILReward_1.toString(), th.applyLiquidationFee(D_coll).mul(C_coll).div(totalColl))
    ...
    */
    ...
    const entireSystemColl_3 = (await activePool.getFIL()).add(await defaultPool.getFIL()).toString()
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). A, B, C top up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    const entireSystemColl_2 = (await activePool.getFIL()).add(await defaultPool.getFIL()).toString()
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    // A, B, C open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingFILReward(bob), B_pendingRewardsAfterL1), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingFILReward(carol), C_pendingRewardsAfterL1), 1000000)
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingFILReward(carol), C_pendingRewardsAfterL2), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingFILReward(dennis), D_pendingRewardsAfterL2), 1000000)
    ...
    // E and F open troves
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(110, 18), extraParams: { from: erin } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(110, 18), extraParams: { from: freddy } })
    ...
    assert.isAtMost(th.getDifference(carol_pendingFILReward, C_collAfterL3.sub(C_collAfterL1)), 1000000)
    assert.isAtMost(th.getDifference(dennis_pendingFILReward, D_collAfterL3.sub(D_collAfterL2)), 1000000)
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    */
    ...
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ extraDebtTokenAmount: dec(100, 18), extraParams: { from: dennis, value: toBN(dec(35, 15)) } })
    ...
    const D_pendingRewardsAfterL2 = D_coll.mul(th.applyLiquidationFee(B_collAfterL1)).div(C_collAfterL1.add(D_coll))
    const C_collAfterL2 = C_collAfterL1.add(C_pendingRewardsAfterL2)
    assert.isAtMost(th.getDifference(await troveManager.getPendingFILReward(carol), C_pendingRewardsAfterL2), 10000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingFILReward(dennis), D_pendingRewardsAfterL2), 10000000)
    ...
    F: 0.0007 FIL
    */
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ extraDebtTokenAmount: dec(100, 18), extraParams: { from: erin, value: toBN(dec(1, 22)) } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ extraDebtTokenAmount: dec(100, 18), extraParams: { from: freddy, value: toBN('700000000000000') } })
