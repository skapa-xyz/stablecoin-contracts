...
contract('StabilityPool - LQTY Rewards', async accounts => {
  ...
  describe("LQTY Rewards", async () => {
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      // Whale opens Trove with 10k ETH
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(10000, 18)), whale, whale, { from: whale, value: dec(10000, 'ether') })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. No liquidations. No front end.", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(30000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(300, 'ether') })
    ...
    1 month passes. L1 decreases P: P = 1e-5 P. L1:   9999.9 DebtToken, 100 ETH
    B makes deposit 9999.9
    1 month passes. L2 decreases P: P =  1e-5 P. L2:  9999.9 DebtToken, 100 ETH
    C makes deposit  9999.9
    1 month passes. L3 decreases P: P = 1e-5 P. L3:  9999.9 DebtToken, 100 ETH
    D makes deposit  9999.9
    1 month passes. L4 decreases P: P = 1e-5 P. L4:  9999.9 DebtToken, 100 ETH
    E makes deposit  9999.9
    1 month passes. L5 decreases P: P = 1e-5 P. L5:  9999.9 DebtToken, 100 ETH
    =========
    F makes deposit 100
    1 month passes. L6 empties the Pool. L6:  10000 DebtToken, 100 ETH
    ...
    it("withdrawFromSP(): Several deposits of 100 DebtToken span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(10000, 18)), whale, whale, { from: whale, value: dec(100, 'ether') })
      ...
      for (const defaulter of fiveDefaulters) {
        ...
        await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount('9999900000000000000000'), defaulter, defaulter, { from: defaulter, value: dec(100, 'ether') })
      }

      // Defaulter 6 withdraws to 10k debt (inc. gas comp)
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(10000, 18)), defaulter_6, defaulter_6, { from: defaulter_6, value: dec(100, 'ether') })
    ...
    it("withdrawFromSP(): Depositors with varying initial deposit withdraw correct LQTY gain. Front ends and kickback rates", async () => {
      ...
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(50000, 18)), defaulter_1, defaulter_1, { from: defaulter_1, value: dec(500, 'ether') })
      await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(20000, 18)), defaulter_2, defaulter_2, { from: defaulter_2, value: dec(200, 'ether') })
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalDebtTokenDeposits(), dec(50000, 18))  // 50k
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalDebtTokenDeposits(), dec(80000, 18))
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalDebtTokenDeposits(), dec(60000, 18))
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalDebtTokenDeposits(), dec(100000, 18))
      ...
      th.assertIsApproximatelyEqual(await stabilityPool.getTotalDebtTokenDeposits(), dec(90000, 18))
      ...
      for (depositor of [A, B, C, D, E]) {
        ...
        const compoundedDebtTokenDeposit = await stabilityPool.getCompoundedDebtTokenDeposit(depositor)
    ...
    1 month passes. L1 depletes P: P = 1e-5*P L1:  9999.9 DebtToken, 1 ETH.  scale = 0
    C makes deposit 10000  via F1
    1 month passes. L2 depletes P: P = 1e-5*P L2:  9999.9 DebtToken, 1 ETH  scale = 1
    D makes deposit 10000 via F1
    1 month passes. L3 depletes P: P = 1e-5*P L3:  9999.9 DebtToken, 1 ETH scale = 1
    E makes deposit 10000 via F1
    1 month passes. L3 depletes P: P = 1e-5*P L4:  9999.9 DebtToken, 1 ETH scale = 2
    A, B, C, D, E withdraw
    ...
    it("withdrawFromSP(): Several deposits of 10k DebtToken span one scale factor change. Depositors withdraw correct LQTY gains", async () => {
      ...
      for (const defaulter of _4_Defaulters) {
        ...
        await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(99999, 17)), defaulter, defaulter, { from: defaulter, value: dec(100, 'ether') })
      }
