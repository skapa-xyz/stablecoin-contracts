...
contract('BorrowerWrappers', async accounts => {
  ...
  it('proxy owner can recover FIL', async () => {
    ...
    // recover FIL
    const gas_Used = th.gasUsed(await borrowerWrappers.transferFIL(alice, amount, { from: alice, gasPrice: GAS_PRICE }))
  ...
  it('claimCollateralAndOpenTrove(): reverts if nothing to claim', async () => {
    ...
    // alice opens Trove
    const { debtTokenAmount, collateral } = await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
    ...
    await assertRevert(
      borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, debtTokenAmount, alice, alice, { from: alice }),
  ...
  it('claimCollateralAndOpenTrove(): without sending any value', async () => {
    // alice opens Trove
    const { debtTokenAmount, netDebt: redeemAmount, collateral } = await openTrove({extraDebtTokenAmount: 0, ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
    // Whale opens Trove
    await openTrove({ extraDebtTokenAmount: redeemAmount, ICR: toBN(dec(5, 18)), extraParams: { from: whale } })
    ...
    // alice claims collateral and re-opens the trove
    await borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, debtTokenAmount, alice, alice, { from: alice })

    assert.equal(await web3.eth.getBalance(proxyAddress), '0')
    th.assertIsApproximatelyEqual(await collSurplusPool.getCollateral(proxyAddress), '0')
    th.assertIsApproximatelyEqual(await debtToken.balanceOf(proxyAddress), debtTokenAmount.mul(toBN(2)))
    assert.equal(await troveManager.getTroveStatus(proxyAddress), 1)
  ...
  it('claimCollateralAndOpenTrove(): sending value in the transaction', async () => {
    // alice opens Trove
    const { debtTokenAmount, netDebt: redeemAmount, collateral } = await openTrove({ extraParams: { from: alice } })
    // Whale opens Trove
    await openTrove({ extraDebtTokenAmount: redeemAmount, ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
    ...
    // alice claims collateral and re-opens the trove
    await borrowerWrappers.claimCollateralAndOpenTrove(th._100pct, debtTokenAmount, alice, alice, { from: alice, value: collateral })
  ...
  it('claimSPRewardsAndRecycle(): only owner can call it', async () => {
    // Whale opens Trove
    await openTrove({ extraDebtTokenAmount: toBN(dec(1850, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
  ...
  it('claimSPRewardsAndRecycle():', async () => {
    ...
    await openTrove({ extraDebtTokenAmount: whaleDeposit, ICR: toBN(dec(4, 18)), extraParams: { from: whale } })
    // Whale deposits 1850 DebtToken in StabilityPool
    await stabilityPool.provideToSP(whaleDeposit, ZERO_ADDRESS, { from: whale })

    // alice opens trove and provides 150 DebtToken to StabilityPool
    const aliceDeposit = toBN(dec(150, 18))
    await openTrove({ extraDebtTokenAmount: aliceDeposit, ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
    await stabilityPool.provideToSP(aliceDeposit, ZERO_ADDRESS, { from: alice })

    // Defaulter Trove opened
    const { debtTokenAmount, netDebt, collateral } = await openTrove({ ICR: toBN(dec(210, 16)), extraParams: { from: defaulter_1 } })
    ...
    const expectedFILGain_A = collateral.mul(aliceDeposit).div(totalDeposits).mul(toBN(dec(995, 15))).div(mv._1e18BN)

    assert.isAtMost(th.getDifference(expectedCompoundedDebtTokenDeposit_A, compoundedDebtTokenDeposit_A), 1000)
    ...
    th.assertIsApproximatelyEqual(depositAfter, depositBefore.sub(expectedDebtTokenLoss_A).add(netDebtChange))
  ...
  it('claimStakingGainsAndRecycle(): only owner can call it', async () => {
    ...
    // Defaulter Trove opened
    const { debtTokenAmount, netDebt, totalDebt, collateral } = await openTrove({ ICR: toBN(dec(210, 16)), extraParams: { from: defaulter_1 } })
  ...
  it('claimStakingGainsAndRecycle(): with only FIL gain', async () => {
    ...
    const expectedFILGain_A = redemptionFee.mul(toBN(dec(150, 18))).div(toBN(dec(2000, 18))).mul(mv._1e18BN).div(price)
    ...
    const proportionalDebtToken = expectedFILGain_A.mul(price).div(ICRBefore)
    const borrowingRate = await troveManagerOriginal.getBorrowingRateWithDecay()
    const netDebtChange = proportionalDebtToken.mul(toBN(dec(1, 18))).div(toBN(dec(1, 18)).add(borrowingRate))
    ...
    const expectedNewDebtTokenGain_A = newBorrowingFee.mul(toBN(dec(150, 18))).div(toBN(dec(2000, 18)))
    ...
    th.assertIsApproximatelyEqual(debtTokenBalanceAfter, debtTokenBalanceBefore.add(expectedNewDebtTokenGain_A))
    // check trove has increased debt by the ICR proportional amount to FIL gain
    th.assertIsApproximatelyEqual(troveDebtAfter, troveDebtBefore.add(proportionalDebtToken), 10000)
    // check trove has increased collateral by the FIL gain
    th.assertIsApproximatelyEqual(troveCollAfter, troveCollBefore.add(expectedFILGain_A))
    // check that ICR remains constant
    th.assertIsApproximatelyEqual(ICRAfter, ICRBefore)
