 ...
 */
 ...
 const makeTrovesInParallel = async (accounts, n) => {
  ...
  const withdrawDebtTokenPromises = activeAccounts.map(account => withdrawDebtTokenFromTrove(account))
 ...
 const withdrawDebtTokenFromTrove = async (account) => {
  await borrowerOperations.withdrawDebtToken(th._100pct, '100000000000000000000', account, account, { from: account })
 }
 ...
 // Sequentially add coll and withdraw debt tokens, 1 account at a time
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      await th.openTrove(contracts, { extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
