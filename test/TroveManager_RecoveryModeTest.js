...
contract('TroveManager - in Recovery Mode', async accounts => {
  ...
  const getOpenTroveDebtTokenAmount = async (totalDebt) => th.getOpenTroveDebtTokenAmount(contracts, totalDebt)
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1FIL:150DebtToken, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1FIL:100DebtToken, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(150, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: only redistributes to active Troves - no offset to Stability Pool", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 DebtToken of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: bob } })
    ...
    // --- TEST ---
    // price drops to 1FIL:100DebtToken, reducing TCR to 117%
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 DebtToken of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraDebtTokenAmount: dec(20000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390DebtToken to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })
    ...
    // check pool DebtToken before liquidation
    const stabilityPoolDebtToken_Before = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
    assert.equal(stabilityPoolDebtToken_Before, '390000000000000000000')
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 DebtToken.
    
    With 390 DebtToken in the StabilityPool, 390 DebtToken should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    DebtTokenLoss: 390DebtToken
    FILGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 DebtToken and 4.074525 ether, the remainders - 1610 DebtToken and 16.820475 ether - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner })

    const aliceDeposit = await stabilityPool.getCompoundedDebtTokenDeposit(alice)
    const aliceFILGain = await stabilityPool.getDepositorFILGain(alice)
    const aliceExpectedFILGain = spDeposit.mul(th.applyLiquidationFee(B_coll)).div(B_totalDebt)

    assert.equal(aliceDeposit.toString(), 0)
    assert.equal(aliceFILGain.toString(), aliceExpectedFILGain)

    /* Now, check redistribution to active Troves. Remainders of 1610 DebtToken and 16.82 ether are distributed.
    ...
    L_Debt = 1610 / 6 = 268.333 DebtToken
    L_FIL = 16.820475 /6 =  2.8034125 ether
    */
    const L_Debt = (await troveManager.L_Debt()).toString()
    const L_FIL = (await troveManager.L_FIL()).toString()

    assert.isAtMost(th.getDifference(L_Debt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))), 100)
    assert.isAtMost(th.getDifference(L_FIL, th.applyLiquidationFee(B_coll.sub(B_coll.mul(spDeposit).div(B_totalDebt)).mul(mv._1e18BN).div(A_coll.add(D_coll)))), 100)
  })
  ...
  it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool DebtToken is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool DebtToken > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: dennis } })

    // Alice deposits DebtToken in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 DebtToken, Alice sole depositor.
    As liquidated debt (250 DebtToken) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240DebtToken
    Alice's expected FIL gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedDebtTokenDeposit(alice)
    const aliceExpectedFILGain = await stabilityPool.getDepositorFILGain(alice)

    assert.isAtMost(th.getDifference(aliceExpectedDeposit.toString(), spDeposit.sub(B_totalDebt)), 2000)
    assert.isAtMost(th.getDifference(aliceExpectedFILGain, th.applyLiquidationFee(B_totalDebt.mul(th.toBN(dec(11, 17))).div(price))), 3000)
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool DebtToken > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(220, 16)), extraDebtTokenAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool DebtToken > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool DebtToken > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool DebtToken > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool DebtToken > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 DebtToken, CR of 200%
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: totalLiquidatedDebt, extraParams: { from: whale } })
  ...
  DebtToken is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool DebtToken < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: dennis } })

    // Alice deposits 1490 DebtToken in the Stability Pool
    ...
    /* Since the pool only contains 100 DebtToken, and Bob's pre-liquidation debt was 250 DebtToken,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool DebtToken < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    // Alice deposits 100 DebtToken in the Stability Pool
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool DebtToken < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    /*  Since Bob's debt (250 DebtToken) is larger than all DebtToken in the Stability Pool, Liquidation won’t happen
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool DebtToken < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool DebtToken < liquidated debt: causes correct Pool offset and FIL gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    assert.equal(aliceExpectedFILGain.toString(), '0')
    ...
    const L_FIL_After = (await troveManager.L_FIL()).toString()

    assert.equal(L_Debt_After, '0')
    assert.equal(L_FIL_After, '0')
  })

  it("liquidate(), with ICR > 110%, and StabilityPool DebtToken < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 DebtToken, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: dec(2000, 18), extraParams: { from: dennis } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(250, 16)), extraDebtTokenAmount: dec(240, 18), extraParams: { from: carol } })
    ...
    //Check SP DebtToken has been completely emptied
    ...
    // Alice provides another 50 DebtToken to pool
  ...
  it("liquidate() with ICR > 110%, and StabilityPool DebtToken < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 DebtToken to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(50, 18), extraParams: { from: whale } })
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 FIL and a debt of 62 DebtToken, and provides 10 DebtToken to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 DebtToken in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or FIL gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(220, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: bob } })

    // Bob sends tokens to Dennis, who has no trove
    await debtToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides 200 DebtToken to SP
    ...
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedDebtTokenDeposit(dennis)).toString()
    ...
    assert.isAtMost(th.getDifference(dennis_FILGain_Before, th.applyLiquidationFee(C_coll)), 1000)
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedDebtTokenDeposit(dennis)).toString()
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(220, 16)), extraDebtTokenAmount: dec(1000, 18), extraParams: { from: whale } })

    const { debtTokenAmount: A_debtTokenAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(300, 18), extraParams: { from: alice } })
    const { debtTokenAmount: B_debtTokenAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(200, 18), extraParams: { from: bob } })
    const { debtTokenAmount: C_debtTokenAmount } = await openTrove({ ICR: toBN(dec(206, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: carol } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 DebtToken of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 DebtToken of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: dec(480, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: B_totalDebt, extraParams: { from: alice } })
    ...
    // Bob re-opens the trove, price 200, total debt 80 DebtToken, ICR = 120% (lowest one)
    // Dennis redeems 30, so Bob has a surplus of (200 * 0.48 - 30) / 200 = 0.33 FIL
    await priceFeed.setPrice('200000000000000000000')
    const { collateral: B_coll_2, netDebt: B_netDebt_2 } = await openTrove({ ICR: toBN(dec(150, 16)), extraDebtTokenAmount: dec(480, 18), extraParams: { from: bob, value: bob_remainingCollateral } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: B_netDebt_2, extraParams: { from: dennis } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 DebtToken of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraDebtTokenAmount: dec(90, 18), extraParams: { from: bob } })
    // Dennis withdraws to 150 DebtToken of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: B_netDebt, extraParams: { from: dennis } })

    // --- TEST ---
    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // Dennis redeems 40, so Bob has a surplus of (200 * 1 - 40) / 200 = 0.8 FIL
    ...
    // Bob re-opens the trove, price 200, total debt 250 DebtToken, ICR = 240% (lowest one)
    ...
    // Alice deposits DebtToken in the Stability Pool
    await openTrove({ ICR: toBN(dec(266, 16)), extraDebtTokenAmount: B_totalDebt_2, extraParams: { from: alice } })
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some DebtToken from their Trove, resulting in different ICRs
    ...
    const { totalDebt: H_totalDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraDebtTokenAmount: dec(5000, 18), extraParams: { from: harry } })
    const liquidationAmount = E_totalDebt.add(F_totalDebt).add(G_totalDebt).add(H_totalDebt)
    await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: liquidationAmount, extraParams: { from: alice } })

    // Alice deposits DebtToken to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, { from: alice })

    // price drops
    // price drops to 1FIL:90DebtToken, reducing TCR below 150%
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1FIL:85DebtToken, reducing TCR below 150%
  ...
  it("liquidateTroves(): does nothing if n = 0", async () => {
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(300, 18), extraParams: { from: carol } })
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    await openTrove({ ICR: toBN(dec(198, 16)), extraDebtTokenAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(184, 16)), extraDebtTokenAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(183, 16)), extraDebtTokenAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(186, 16)), extraDebtTokenAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
    ...
    // Check Stability Pool has 500 DebtToken
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraDebtTokenAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    const { collateral: d1_coll, totalDebt: d1_totalDebt } = await openTrove({ ICR: toBN(dec(198, 16)), extraDebtTokenAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    const { collateral: d2_coll, totalDebt: d2_totalDebt } = await openTrove({ ICR: toBN(dec(184, 16)), extraDebtTokenAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    const { collateral: d3_coll, totalDebt: d3_totalDebt } = await openTrove({ ICR: toBN(dec(183, 16)), extraDebtTokenAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    const { collateral: d4_coll, totalDebt: d4_totalDebt } = await openTrove({ ICR: toBN(dec(166, 16)), extraDebtTokenAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty', async () => {
    ...
    // Confirm 0 DebtToken in Stability Pool
    assert.equal((await stabilityPool.getTotalDebtTokenDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    // Whale adds DebtToken to SP
    const spDeposit = F_totalDebt.add(G_totalDebt)
    await openTrove({ ICR: toBN(dec(285, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    ...
    // Confirm DebtToken in Stability Pool
    assert.equal((await stabilityPool.getTotalDebtTokenDeposits()).toString(), spDeposit.toString())
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(285, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    const entireColl_A = (await troveManager.Troves(alice))[1].add(await troveManager.getPendingFILReward(alice))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { debtTokenAmount: debtTokenAmountD } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: dennis } })
    const { debtTokenAmount: debtTokenAmountE } = await openTrove({ ICR: toBN(dec(133, 16)), extraParams: { from: erin } })
    const { debtTokenAmount: debtTokenAmountF } = await openTrove({ ICR: toBN(dec(111, 16)), extraParams: { from: freddy } })

    // Check list size is 4
    assert.equal((await sortedTroves.getSize()).toString(), '4')

    // Check token balances before
    assert.equal((await debtToken.balanceOf(dennis)).toString(), debtTokenAmountD)
    assert.equal((await debtToken.balanceOf(erin)).toString(), debtTokenAmountE)
    assert.equal((await debtToken.balanceOf(freddy)).toString(), debtTokenAmountF)
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and FIL gain", async () => {
    // Whale provides DebtToken to the SP
    const { debtTokenAmount: W_debtTokenAmount } = await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(4000, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(W_debtTokenAmount, ZERO_ADDRESS, { from: whale })

    const { debtTokenAmount: A_debtTokenAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(191, 16)), extraDebtTokenAmount: dec(40, 18), extraParams: { from: alice } })
    const { debtTokenAmount: B_debtTokenAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(240, 18), extraParams: { from: bob } })
    ...
    await stabilityPool.provideToSP(A_debtTokenAmount, ZERO_ADDRESS, { from: alice })
    await stabilityPool.provideToSP(B_debtTokenAmount, ZERO_ADDRESS, { from: bob })

    const totalDeposit = W_debtTokenAmount.add(A_debtTokenAmount).add(B_debtTokenAmount)
    ...
    // Check DebtToken in Pool
    assert.equal((await stabilityPool.getTotalDebtTokenDeposits()).toString(), totalDeposit)
    ...
    Total DebtToken in Pool: 680 DebtToken

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 DebtToken
    Total liquidated FIL = 1 + 3 + 1 = 5 FIL

    Whale DebtToken Loss: 500 * (400/680) = 294.12 DebtToken
    Alice DebtToken Loss:  500 *(40/680) = 29.41 DebtToken
    Bob DebtToken Loss: 500 * (240/680) = 176.47 DebtToken

    Whale remaining deposit: (400 - 294.12) = 105.88 DebtToken
    Alice remaining deposit: (40 - 29.41) = 10.59 DebtToken
    Bob remaining deposit: (240 - 176.47) = 63.53 DebtToken

    Whale FIL Gain: 5*0.995 * (400/680) = 2.93 FIL
    Alice FIL Gain: 5*0.995 *(40/680) = 0.293 FIL
    Bob FIL Gain: 5*0.995 * (240/680) = 1.76 FIL

    Total remaining deposits: 180 DebtToken
    Total FIL gain: 5*0.995 FIL */
    ...
    const whale_Deposit_After = (await stabilityPool.getCompoundedDebtTokenDeposit(whale)).toString()
    const alice_Deposit_After = (await stabilityPool.getCompoundedDebtTokenDeposit(alice)).toString()
    const bob_Deposit_After = (await stabilityPool.getCompoundedDebtTokenDeposit(bob)).toString()

    const whale_FILGain = (await stabilityPool.getDepositorFILGain(whale)).toString()
    const alice_FILGain = (await stabilityPool.getDepositorFILGain(alice)).toString()
    const bob_FILGain = (await stabilityPool.getDepositorFILGain(bob)).toString()

    const liquidatedDebt = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    const liquidatedColl = A_coll.add(B_coll).add(C_coll)
    assert.isAtMost(th.getDifference(whale_Deposit_After, W_debtTokenAmount.sub(liquidatedDebt.mul(W_debtTokenAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(alice_Deposit_After, A_debtTokenAmount.sub(liquidatedDebt.mul(A_debtTokenAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(bob_Deposit_After, B_debtTokenAmount.sub(liquidatedDebt.mul(B_debtTokenAmount).div(totalDeposit))), 100000)

    assert.isAtMost(th.getDifference(whale_FILGain, th.applyLiquidationFee(liquidatedColl).mul(W_debtTokenAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(alice_FILGain, th.applyLiquidationFee(liquidatedColl).mul(A_debtTokenAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(bob_FILGain, th.applyLiquidationFee(liquidatedColl).mul(B_debtTokenAmount).div(totalDeposit)), 2000)
    ...
    assert.isAtMost(th.getDifference(total_debtTokenInSP, totalDeposit.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_FILinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or FIL gain", async () => {
    // Whale provides 400 DebtToken to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(400, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(dec(400, 18), ZERO_ADDRESS, { from: whale })

    await openTrove({ ICR: toBN(dec(182, 16)), extraDebtTokenAmount: dec(170, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraDebtTokenAmount: dec(300, 18), extraParams: { from: bob } })
    ...
    // Check DebtToken and FIL in Pool  before
    ...
    assert.equal(debtTokenInSP_Before, dec(800, 18))
    assert.equal(FILinSP_Before, '0')
    ...
    // Check DebtToken and FIL in Pool after
    ...
    const whale_FILGain_After = (await stabilityPool.getDepositorFILGain(whale)).toString()
    ...
    assert.equal(whale_FILGain_After, '0')
    assert.equal(alice_FILGain_After, '0')
    assert.equal(bob_FILGain_After, '0')
  })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    That leaves 50 DebtToken in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10)
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array", async () => {
    ...
    // Whale provides DebtToken to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(220, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: D_totalDebt, extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 DebtToken in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 DebtToken in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10)

    // Expect system debt reduced by 203 DebtToken and system coll 2.3 FIL
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    // Whale provides DebtToken to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(240, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 DebtToken in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10)

    const [liquidatedDebt, liquidatedColl, collGasComp, debtTokenGasComp] = th.getEmittedLiquidationValues(liquidationTx)
    ...
    assert.equal(debtTokenGasComp.toString(), dec(400, 18))
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: alice } })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    const spDeposit = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(E_totalDebt).add(F_totalDebt)
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(426, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(426, 16)), extraDebtTokenAmount: A_totalDebt, extraParams: { from: whale } })
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 DebtToken and system coll by 2 FIL
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool DebtToken > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraDebtTokenAmount: dec(500, 18), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(282, 16)), extraDebtTokenAmount: dec(500, 18), extraParams: { from: freddy } })

    // Whale provides 1000 DebtToken to the SP
    const spDeposit = A_totalDebt.add(C_totalDebt).add(D_totalDebt)
    await openTrove({ ICR: toBN(dec(219, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisColl_After = (await troveManager.Troves(dennis))[1].add(await troveManager.getPendingFILReward(dennis))  
    const bobColl_After = (await troveManager.Troves(bob))[1].add(await troveManager.getPendingFILReward(bob))
    const carolColl_After = (await troveManager.Troves(carol))[1].add(await troveManager.getPendingFILReward(carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    await openTrove({ ICR: toBN(dec(245, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    // Whale withdraws entire deposit, and re-deposits 132 DebtToken
    ...
    // Expected Stability Pool to fully absorb B (92 DebtToken + 10 virtual debt), 
    // but not E as there are not enough funds in Stability Pool
    
    const stabilityBefore = await stabilityPool.getTotalDebtTokenDeposits()
    ...
    const stabilityAfter = await stabilityPool.getTotalDebtTokenDeposits()
  ...
  it('batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    // Whale opens trove and adds 220 DebtToken to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(285, 16)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
