...
contract('DefaultPool', async accounts => {
  ...
  it('sendFILToActivePool(): fails if receiver cannot receive FIL', async () => {
    ...
    const sendFILData = th.getTransactionData('sendFILToActivePool(uint256)', [web3.utils.toHex(amount)])
    await th.assertRevert(mockTroveManager.forward(defaultPool.address, sendFILData, { from: owner }), 'DefaultPool: sending FIL failed')
  })
})
