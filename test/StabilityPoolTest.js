...
contract('StabilityPool', async accounts => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    it("provideToSP(): increases the Stability Pool DebtToken balance", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(200), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
    ...
    it("provideToSP(): increases totalDebtTokenDeposits by correct amount", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
    ...
    it('provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked', async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: dec(50, 'ether') } })
      const whaleDebtToken = await debtToken.balanceOf(whale)
      await stabilityPool.provideToSP(whaleDebtToken, frontEnd_1, { from: whale })

      // 2 Troves opened, each withdraws minimum debt
      await openTrove({ extraDebtTokenAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1, } })
      await openTrove({ extraDebtTokenAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2, } })

      // Alice makes Trove and withdraws 100 DebtToken
      await openTrove({ extraDebtTokenAmount: toBN(dec(100, 18)), ICR: toBN(dec(5, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1, value: dec(50, 'ether') } })
      await openTrove({ extraDebtTokenAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2, value: dec(50, 'ether') } })
      await openTrove({ extraDebtTokenAmount: 0, ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_3, value: dec(50, 'ether') } })

      // --- TEST ---

      // Alice makes deposit #1: 150 DebtToken
      await openTrove({ extraDebtTokenAmount: toBN(dec(250, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      ...
      await troveManager.liquidate(defaulter_1, { from: owner })  // 180 DebtToken closed
      await troveManager.liquidate(defaulter_2, { from: owner }) // 180 DebtToken closed

      const alice_compoundedDeposit_1 = await stabilityPool.getCompoundedDebtTokenDeposit(alice)
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await stabilityPool.provideToSP(dec(427, 18), frontEnd_1, { from: alice })

      // Defaulter 3 Trove is closed
      await troveManager.liquidate(defaulter_3, { from: owner })

      const alice_compoundedDeposit_2 = await stabilityPool.getCompoundedDebtTokenDeposit(alice)
    ...
    it("provideToSP(): reverts if user tries to provide more than their DebtToken balance", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice, value: dec(50, 'ether') } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob, value: dec(50, 'ether') } })
      const aliceDebtTokenBal = await debtToken.balanceOf(alice)
      const bobDebtTokenBal = await debtToken.balanceOf(bob)

      // Alice, attempts to deposit 1 wei more than her balance

      const aliceTxPromise = stabilityPool.provideToSP(aliceDebtTokenBal.add(toBN(1)), frontEnd_1, { from: alice })
      await assertRevert(aliceTxPromise, "revert")

      // Bob, attempts to deposit 235534 more than his balance

      const bobTxPromise = stabilityPool.provideToSP(bobDebtTokenBal.add(toBN(dec(235534, 18))), frontEnd_1, { from: bob })
    ...
    it("provideToSP(): reverts if cannot receive FIL Gain", async () => {
      ...
      await th.assertRevert(nonPayable.forward(stabilityPool.address, txData2), 'StabilityPool: sending FIL failed')
    })
    ...
    it("provideToSP(): doesn't impact other users' deposits or FIL gains", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // D opens a trove
      await openTrove({ extraDebtTokenAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      ...
      const alice_debtTokenDeposit_Before = (await stabilityPool.getCompoundedDebtTokenDeposit(alice)).toString()
      const bob_debtTokenDeposit_Before = (await stabilityPool.getCompoundedDebtTokenDeposit(bob)).toString()
      const carol_debtTokenDeposit_Before = (await stabilityPool.getCompoundedDebtTokenDeposit(carol)).toString()

      const alice_FILGain_Before = (await stabilityPool.getDepositorFILGain(alice)).toString()
      ...
      assert.equal((await stabilityPool.getCompoundedDebtTokenDeposit(dennis)).toString(), dec(1000, 18))

      const alice_debtTokenDeposit_After = (await stabilityPool.getCompoundedDebtTokenDeposit(alice)).toString()
      const bob_debtTokenDeposit_After = (await stabilityPool.getCompoundedDebtTokenDeposit(bob)).toString()
      const carol_debtTokenDeposit_After = (await stabilityPool.getCompoundedDebtTokenDeposit(carol)).toString()
    ...
    it("provideToSP(): doesn't impact system debt, collateral or TCR", async () => {
      ...
      // D opens a trove
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
    ...
    it("provideToSP(): doesn't impact any troves, including the caller's trove", async () => {
      ...
      // D opens a trove
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
    ...
    it("provideToSP(): doesn't protect the depositor's trove from liquidation", async () => {
      ...
      assert.equal((await stabilityPool.getCompoundedDebtTokenDeposit(bob)).toString(), dec(1000, 18))
    ...
    it("provideToSP(), new deposit: when SP > 0, triggers LQTY reward event - increases the sum G", async () => {
      ...
      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(), new deposit: sets the correct front end tag", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive any LQTY rewards", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      // A, B, C, open troves 
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(4000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), new eligible deposit: tagged front end receives LQTY rewards", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("provideToSP(), new deposit: depositor does not receive FIL gains", async () => {
      ...
      // C, D open troves
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(2000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      // --- TEST ---

      // get current FIL balances
      const A_FILBalance_Before = await web3.eth.getBalance(A)
      const B_FILBalance_Before = await web3.eth.getBalance(B)
      const C_FILBalance_Before = await web3.eth.getBalance(C)
      const D_FILBalance_Before = await web3.eth.getBalance(D)
      ...
      const A_FILBalance_After = await web3.eth.getBalance(A)
      const B_FILBalance_After = await web3.eth.getBalance(B)
      const C_FILBalance_After = await web3.eth.getBalance(C)
      const D_FILBalance_After = await web3.eth.getBalance(D)
    ...
    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive FIL gains", async () => {
      ...
      // C, D open troves
      await openTrove({ extraDebtTokenAmount: toBN(dec(4000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("provideToSP(), topup from different front end: doesn't change the front end tag", async () => {
      ...
      // A, B, C open troves 
      await openTrove({ extraDebtTokenAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(), topup: tagged front end's stake increases", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(300, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("provideToSP(), topup: tagged front end's snapshots update", async () => {
      ...
      // A, B, C, open troves 
      await openTrove({ extraDebtTokenAmount: toBN(dec(200, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(400, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(600, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("provideToSP(): reverts if user is a registered front end", async () => {
      // C, D, E, F open troves 
      await openTrove({ extraDebtTokenAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("withdrawFromSP(): reverts when user has no active deposit", async () => {
      await openTrove({ extraDebtTokenAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    it("withdrawFromSP(): partial retrieval - retrieves correct DebtToken amount and the entire FIL Gain, and updates deposit", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(1, 24)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(15000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      const liquidationTX_1 = await troveManager.liquidate(defaulter_1, { from: owner })  // 170 DebtToken closed
      const liquidationTX_2 = await troveManager.liquidate(defaulter_2, { from: owner }) // 170 DebtToken closed
      ...
      const expectedDebtTokenLoss_A = (liquidatedDebt_1.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))
        .add(liquidatedDebt_2.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18))))

      const expectedCompoundedDebtTokenDeposit_A = toBN(dec(15000, 18)).sub(expectedDebtTokenLoss_A)
      const compoundedDebtTokenDeposit_A = await stabilityPool.getCompoundedDebtTokenDeposit(alice)

      assert.isAtMost(th.getDifference(expectedCompoundedDebtTokenDeposit_A, compoundedDebtTokenDeposit_A), 100000)
    ...
    it("withdrawFromSP(): full retrieval - leaves the correct amount of DebtToken in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 DebtToken in StabilityPool
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await stabilityPool.provideToSP(dec(185000, 18), frontEnd_1, { from: whale })
    ...
    it("withdrawFromSP(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero FIL", async () => {
      ...
      // Alice retrieves all of her entitled DebtToken:
      await stabilityPool.withdrawFromSP(dec(15000, 18), { from: alice })
      assert.equal(await stabilityPool.getDepositorFILGain(alice), 0)
      ...
      const txPromise_A = stabilityPool.withdrawFILGainToTrove(alice, alice, { from: alice })
    ...
    it("withdrawFromSP(): decreases StabilityPool FIL", async () => {
      ...
      const liquidationTx_1 = await troveManager.liquidate(defaulter_1, { from: owner })  // 180 DebtToken closed
      ...
      const aliceExpectedFILGain = liquidatedColl.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)))
    ...
    it("withdrawFromSP(): increases depositor's DebtToken token balance by the expected amount", async () => {
      // Whale opens trove 
      await openTrove({ extraDebtTokenAmount: toBN(dec(100000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      */
      ...
      await borrowerOperations.withdrawDebtToken(th._100pct, dec(5000, 18), bob, bob, { from: bob })
    ...
    it("withdrawFromSP(): doesn't impact other users Stability deposits or FIL gains", async () => {
      ...
      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      const A_expectedBalance = A_FILBalBefore.sub((toBN(th.gasUsed(tx) * GAS_PRICE)))
    ...
    it("withdrawFromSP(): withdrawing 0 FIL Gain does not alter the caller's FIL balance, their trove collateral, or the FIL  in the Stability Pool", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      await stabilityPool.provideToSP(dec(100, 18), frontEnd_1, { from: dennis })

      // Check Dennis has 0 FILGain
      const dennis_FILGain = (await stabilityPool.getDepositorFILGain(dennis)).toString()
      assert.equal(dennis_FILGain, '0')

      const dennis_FILBalance_Before = (web3.eth.getBalance(dennis)).toString()
      ...
      const dennis_FILBalance_After = (web3.eth.getBalance(dennis)).toString()
    ...
    it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      ...
      const alice_debtToken_Balance_Before = await debtToken.balanceOf(alice)
      const bob_debtToken_Balance_Before = await debtToken.balanceOf(bob)

      const alice_Deposit_Before = await stabilityPool.getCompoundedDebtTokenDeposit(alice)
      ...
      const bob_expectedDebtTokenBalance = (bob_debtToken_Balance_Before.add(bob_Deposit_Before)).toString()
      const bob_debtToken_Balance_After = (await debtToken.balanceOf(bob)).toString()
      ...
      const alice_expectedDebtTokenBalance = (alice_debtToken_Balance_Before.add(alice_Deposit_Before)).toString()
      const alice_debtToken_Balance_After = (await debtToken.balanceOf(alice)).toString()
      ...
      const expectedDebtTokenInSP = (DebtTokenInSP_Before.sub(alice_Deposit_Before).sub(bob_Deposit_Before)).toString()
      const DebtTokenInSP_After = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
    ...
    it("withdrawFromSP(): caller can withdraw full deposit and FIL gain during Recovery Mode", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      // Price halves
      await priceFeed.setPrice(dec(200, 18))

      // A, B, C open troves and make Stability Pool deposits
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(20000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: bob } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(4, 18)), extraParams: { from: carol } })
      ...
      const alice_debtToken_Balance_Before = await debtToken.balanceOf(alice)
      const bob_debtToken_Balance_Before = await debtToken.balanceOf(bob)
      const carol_debtToken_Balance_Before = await debtToken.balanceOf(carol)

      const alice_FIL_Balance_Before = web3.utils.toBN(await web3.eth.getBalance(alice))
      const bob_FIL_Balance_Before = web3.utils.toBN(await web3.eth.getBalance(bob))
      const carol_FIL_Balance_Before = web3.utils.toBN(await web3.eth.getBalance(carol))

      const alice_Deposit_Before = await stabilityPool.getCompoundedDebtTokenDeposit(alice)
      const bob_Deposit_Before = await stabilityPool.getCompoundedDebtTokenDeposit(bob)
      const carol_Deposit_Before = await stabilityPool.getCompoundedDebtTokenDeposit(carol)

      const alice_FILGain_Before = await stabilityPool.getDepositorFILGain(alice)
      const bob_FILGain_Before = await stabilityPool.getDepositorFILGain(bob)
      const carol_FILGain_Before = await stabilityPool.getDepositorFILGain(carol)
      ...
      const bob_expectedDebtTokenBalance = (bob_debtToken_Balance_Before.add(bob_Deposit_Before)).toString()
      const carol_expectedDebtTokenBalance = (carol_debtToken_Balance_Before.add(carol_Deposit_Before)).toString()

      const alice_debtToken_Balance_After = (await debtToken.balanceOf(alice)).toString()
 
      const bob_debtToken_Balance_After = (await debtToken.balanceOf(bob)).toString()
      const carol_debtToken_Balance_After = (await debtToken.balanceOf(carol)).toString()
      ...
      const alice_expectedFILBalance = (alice_FIL_Balance_Before.add(alice_FILGain_Before)).toString()
      const bob_expectedFILBalance = (bob_FIL_Balance_Before.add(bob_FILGain_Before)).toString()
      const carol_expectedFILBalance = (carol_FIL_Balance_Before.add(carol_FILGain_Before)).toString()

      const alice_FILBalance_After = (await web3.eth.getBalance(alice)).toString()
      const bob_FILBalance_After = (await web3.eth.getBalance(bob)).toString()
      const carol_FILBalance_After = (await web3.eth.getBalance(carol)).toString()
      ...
      const expectedDebtTokenInSP = (DebtTokenInSP_Before
    ...
    it("getDepositorFILGain(): depositor does not earn further FIL gains from liquidations while their compounded deposit == 0: ", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_2 } })
      ...
      const alice_Deposit = (await stabilityPool.getCompoundedDebtTokenDeposit(alice)).toString()
      const bob_Deposit = (await stabilityPool.getCompoundedDebtTokenDeposit(bob)).toString()

      assert.equal(alice_Deposit, '0')
      assert.equal(bob_Deposit, '0')

      // Get FIL gain for A and B
      const alice_FILGain_1 = (await stabilityPool.getDepositorFILGain(alice)).toString()
      const bob_FILGain_1 = (await stabilityPool.getDepositorFILGain(bob)).toString()
      ...
      const alice_FILGain_2 = (await stabilityPool.getDepositorFILGain(alice)).toString()
      const bob_FILGain_2 = (await stabilityPool.getDepositorFILGain(bob)).toString()
      ...
      const alice_FILGain_3 = (await stabilityPool.getDepositorFILGain(alice)).toString()
      const bob_FILGain_3 = (await stabilityPool.getDepositorFILGain(bob)).toString()
    ...
    it("withdrawFromSP(): triggers LQTY reward event - increases the sum G", async () => {
      ...
      // A, B, C open troves
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's stake decreases", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: F } })
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's snapshots update", async () => {
      ...
      // A, B, C, open troves 
      await openTrove({ extraDebtTokenAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(60000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })

      // D opens trove
      await openTrove({ extraDebtTokenAmount: toBN(dec(100000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal: removes deposit's front end tag", async () => {
      ...
      //C, D open troves
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(20000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: E } })
      ...
      // C, D open troves
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: D } })
    ...
    it("withdrawFromSP(), full withdrawal that reduces front end stake to 0: zero’s the front end’s snapshots", async () => {
      ...
      // E opens trove and makes a deposit
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_3, { from: E })
      ...
      // A, B open troves
      await openTrove({ extraDebtTokenAmount: toBN(dec(30000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(10100, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await stabilityPool.provideToSP(dec(10000, 18), frontEnd_1, { from: A })
    ...
    it("withdrawFILGainToTrove(): reverts when user has no active deposit", async () => {
      ...
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      const txAlice = await stabilityPool.withdrawFILGainToTrove(alice, alice, { from: alice })
      assert.isTrue(txAlice.receipt.status)

      const txPromise_B = stabilityPool.withdrawFILGainToTrove(bob, bob, { from: bob })
      await th.assertRevert(txPromise_B)
    })
    ...
    it("withdrawFILGainToTrove(): Applies DebtTokenLoss to user's deposit, and redirects FIL reward to user's Trove", async () => {
      ...
      const FILGain_A = await stabilityPool.getDepositorFILGain(alice)
      const compoundedDeposit_A = await stabilityPool.getCompoundedDebtTokenDeposit(alice)
      ...
      const expectedDebtTokenLoss_A = liquidatedDebt.mul(toBN(dec(15000, 18))).div(toBN(dec(200000, 18)))
      ...
      await stabilityPool.withdrawFILGainToTrove(alice, alice, { from: alice })
    ...
    it("withdrawFILGainToTrove(): reverts if it would leave trove with ICR < MCR", async () => {
      ...
      // Alice makes deposit #1: 15000 DebtToken
      await openTrove({ extraDebtTokenAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await stabilityPool.provideToSP(dec(15000, 18), frontEnd_1, { from: alice })
      ...
      await assertRevert(stabilityPool.withdrawFILGainToTrove(alice, alice, { from: alice }),
    ...
    it("withdrawFILGainToTrove(): All depositors are able to withdraw their FIL gain from the SP to their Trove", async () => {
      ...
      // All depositors attempt to withdraw
      const tx1 = await stabilityPool.withdrawFILGainToTrove(alice, alice, { from: alice })
      assert.isTrue(tx1.receipt.status)
      const tx2 = await stabilityPool.withdrawFILGainToTrove(bob, bob, { from: bob })
      assert.isTrue(tx1.receipt.status)
      const tx3 = await stabilityPool.withdrawFILGainToTrove(carol, carol, { from: carol })
      assert.isTrue(tx1.receipt.status)
      const tx4 = await stabilityPool.withdrawFILGainToTrove(dennis, dennis, { from: dennis })
      assert.isTrue(tx1.receipt.status)
      const tx5 = await stabilityPool.withdrawFILGainToTrove(erin, erin, { from: erin })
      assert.isTrue(tx1.receipt.status)
      const tx6 = await stabilityPool.withdrawFILGainToTrove(flyn, flyn, { from: flyn })
      assert.isTrue(tx1.receipt.status)
    })
    ...
    it("withdrawFILGainToTrove(): All depositors withdraw, each withdraw their correct FIL gain", async () => {
      ...
      */
      ...
      await stabilityPool.withdrawFILGainToTrove(bob, bob, { from: bob })
      ...
      await stabilityPool.withdrawFILGainToTrove(carol, carol, { from: carol })
      ...
      await stabilityPool.withdrawFILGainToTrove(dennis, dennis, { from: dennis })
      ...
      await stabilityPool.withdrawFILGainToTrove(erin, erin, { from: erin })
      ...
      await stabilityPool.withdrawFILGainToTrove(flyn, flyn, { from: flyn })
    ...
    it("withdrawFILGainToTrove(): reverts if user has no trove", async () => {
     ...
     await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: defaulter_1 } })
      ...
      await th.assertRevert(stabilityPool.withdrawFILGainToTrove(dennis, dennis, { from: dennis }), "caller must have an active trove to withdraw FILGain to")
    })
    ...
    it("withdrawFILGainToTrove(): triggers LQTY reward event - increases the sum G", async () => {
      ...
      assert.isTrue((await stabilityPool.getDepositorFILGain(B)).gt(ZERO))
    ...
    it("withdrawFILGainToTrove(), partial withdrawal: doesn't change the front end tag", async () => {
      ...
      assert.isTrue((await stabilityPool.getDepositorFILGain(B)).gt(ZERO))
      assert.isTrue((await stabilityPool.getDepositorFILGain(C)).gt(ZERO))
    ...
    it("withdrawFILGainToTrove(): reverts when depositor has no FIL gain", async () => {
      ...
      await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
      await openTrove({ extraDebtTokenAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await stabilityPool.provideToSP(dec(3000, 18), ZERO_ADDRESS, { from: E })

      // Confirm A, B, C have zero FIL gain
      assert.equal(await stabilityPool.getDepositorFILGain(A), '0')
      assert.equal(await stabilityPool.getDepositorFILGain(B), '0')
      assert.equal(await stabilityPool.getDepositorFILGain(C), '0')

      // Check withdrawFILGainToTrove reverts for A, B, C
      const txPromise_A = stabilityPool.withdrawFILGainToTrove(A, A, { from: A })
      const txPromise_B = stabilityPool.withdrawFILGainToTrove(B, B, { from: B })
      const txPromise_C = stabilityPool.withdrawFILGainToTrove(C, C, { from: C })
      const txPromise_D = stabilityPool.withdrawFILGainToTrove(D, D, { from: D })
    ...
    it("registerFrontEnd(): reverts if address has a non-zero deposit already", async () => {
      // C, D, E open troves 
      await openTrove({ extraDebtTokenAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      await openTrove({ extraDebtTokenAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
