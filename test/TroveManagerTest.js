...
contract('TroveManager', async accounts => {
  ...
  const getOpenTroveTotalDebt = async (debtTokenAmount) => th.getOpenTroveTotalDebt(contracts, debtTokenAmount)
  ...
  it("liquidate(): updates the L_FIL and L_Debt reward-per-unit-staked totals", async () => {
    ...
    const L_FIL_expected_1 = th.applyLiquidationFee(C_collateral).mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    ...
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawDebtToken({ ICR: toBN(dec(111, 16)), extraParams: { from: bob } })
   ...
   L_Debt = (180 / 20) + (890 / 10) = 98 DebtToken */
    ...
    const L_FIL_expected_2 = L_FIL_expected_1.add(th.applyLiquidationFee(B_collateral.add(B_collateral.mul(L_FIL_expected_1).div(mv._1e18BN))).mul(mv._1e18BN).div(A_collateral))
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    ...
    await openTrove({ ICR: toBN(dec(4, 18)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
  ...
  it("liquidate(): does not affect the SP deposit or FIL gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(3, 18)), extraDebtTokenAmount: spDeposit, extraParams: { from: bob } })
    const { C_totalDebt, C_collateral } = await openTrove({ ICR: toBN(dec(218, 16)), extraDebtTokenAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
  ...
  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or FIL gain", async () => {
    ...
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraDebtTokenAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(218, 16)), extraDebtTokenAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and FIL gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraDebtTokenAmount: A_spDeposit, extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(218, 16)), extraDebtTokenAmount: B_spDeposit, extraParams: { from: bob } })
    const { collateral: C_collateral, totalDebt: C_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraDebtTokenAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(bob_FILGain_Before, th.applyLiquidationFee(C_collateral)), 1000)
    ...
    assert.isAtMost(th.getDifference(alice_FILGain_After, th.applyLiquidationFee(B_collateral).mul(A_spDeposit).div(totalDeposits)), 1000000)
    ...
    assert.isAtMost(th.getDifference(bob_FILGain_After, bob_FILGain_Before.add(th.applyLiquidationFee(B_collateral).mul(bob_Deposit_Before).div(totalDeposits))), 1000000)
  })

  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { debtTokenAmount: A_debtTokenAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraDebtTokenAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { debtTokenAmount: B_debtTokenAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraDebtTokenAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { debtTokenAmount: C_debtTokenAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraDebtTokenAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await openTrove({ ICR: toBN(dec(8, 18)), extraDebtTokenAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(221, 16)), extraDebtTokenAmount: toBN(dec(100, 18)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraDebtTokenAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraDebtTokenAmount: dec(1, 24), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraDebtTokenAmount: toBN(dec(500, 18)), extraParams: { from: whale } })
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and FIL gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraDebtTokenAmount: whaleDeposit, extraParams: { from: whale } })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraDebtTokenAmount: A_deposit, extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraDebtTokenAmount: B_deposit, extraParams: { from: bob } })
    ...
    assert.isAtMost(th.getDifference(whale_FILGain, th.applyLiquidationFee(liquidatedColl).mul(whaleDeposit).div(totalDeposits)), 100000)
    assert.isAtMost(th.getDifference(alice_FILGain, th.applyLiquidationFee(liquidatedColl).mul(A_deposit).div(totalDeposits)), 100000)
    assert.isAtMost(th.getDifference(bob_FILGain, th.applyLiquidationFee(liquidatedColl).mul(B_deposit).div(totalDeposits)), 100000)
  ...
  it("liquidateTroves(): when SP > 0, triggers LQTY reward event - increases the sum G", async () => {
    ...
    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraDebtTokenAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraDebtTokenAmount: spDeposit, extraParams: { from: whale } })
    ...
    th.assertIsApproximatelyEqual((await stabilityPool.getTotalDebtTokenDeposits()).toString(), spDeposit.sub(A_debt).sub(B_debt))
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraDebtTokenAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('redeemCollateral(): cancels the provided DebtToken with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraDebtTokenAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraDebtTokenAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraDebtTokenAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraDebtTokenAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    const expectedReceivedFIL = expectedTotalFILDrawn.sub(toBN(FILFee)).sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)) // substract gas used for troveManager.redeemCollateral from expected received FIL
    ...
    assert.equal(dennis_debtTokenBalance_After, dennis_debtTokenBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraDebtTokenAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraDebtTokenAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraDebtTokenAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST --- 

    // open trove from redeemer.  Redeemer has highest ICR (100FIL, 100 DebtToken), 20000%
    const { debtTokenAmount: F_debtTokenAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraDebtTokenAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraDebtTokenAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraDebtTokenAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraDebtTokenAmount: dec(20, 18), extraParams: { from: carol } })
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(10000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })
    ...
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, debtTokenRedemption, th._100pct)
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(6000, 18)), A, A, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(20000, 18)), B, B, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveDebtTokenAmount(dec(30000, 18)), C, C, { from: C, value: dec(1000, 'ether') })
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraDebtTokenAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraDebtTokenAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraDebtTokenAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    const expectedReceivedFIL = expectedTotalFILDrawn.sub(FILFee).sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)) // substract gas used for troveManager.redeemCollateral from expected received FIL

    th.assertIsApproximatelyEqual(expectedReceivedFIL, receivedFIL)

    const dennis_debtTokenBalance_After = (await debtToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_debtTokenBalance_After, dennis_debtTokenBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    await openTrove({ ICR: toBN(dec(133, 16)), extraDebtTokenAmount: amount, extraParams: { from: bob } })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { debtTokenAmount: B_debtTokenAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraDebtTokenAmount: A_debt, extraParams: { from: bob } })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraDebtTokenAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraDebtTokenAmount: dec(10, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(500, 18), extraParams: { from: alice } })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(80, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(90, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: C } })
    ...
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedDebtTokenRedemption, lessThan5pct), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)  // artificially zero the baseRate
    
    // Max fee is 1%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedDebtTokenRedemption, dec(1, 16)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

     // Max fee is 3.754%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedDebtTokenRedemption, dec(3754, 13)), "Fee exceeded provided maximum")
  
    await troveManager.setBaseRate(0)

    // Max fee is 0.5%
    await assertRevert(th.redeemCollateralAndGetTxObject(A, contracts, attemptedDebtTokenRedemption, dec(5, 15)), "Fee exceeded provided maximum")
  })

  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraDebtTokenAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraDebtTokenAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    ...
    const tx1 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedDebtTokenRedemption, GAS_PRICE, slightlyMoreThanFee)
    ...
    const tx2 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedDebtTokenRedemption, GAS_PRICE, exactSameFee)
    assert.isTrue(tx2.receipt.status)

    await troveManager.setBaseRate(0)

     // Max fee is 10%
    const tx3 = await th.redeemCollateralAndGetTxObject(B, contracts, attemptedDebtTokenRedemption, GAS_PRICE, dec(1, 17))
    assert.isTrue(tx3.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 37.659%
    const tx4 = await th.redeemCollateralAndGetTxObject(A, contracts, attemptedDebtTokenRedemption, GAS_PRICE, dec(37659, 13))
    assert.isTrue(tx4.receipt.status)

    await troveManager.setBaseRate(0)

    // Max fee is 100%
    const tx5 = await th.redeemCollateralAndGetTxObject(C, contracts, attemptedDebtTokenRedemption, GAS_PRICE, dec(1, 18))
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or FIL gain of redeemed-from troves", async () => {
    ...
    // B, C, D, F open trove
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraDebtTokenAmount: dec(200, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraDebtTokenAmount: dec(400, 18), extraParams: { from: dennis } })
    const { totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: flyn } })
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: redemptionAmount, extraParams: { from: alice } })
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedDebtTokenDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedDebtTokenDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedDebtTokenDeposit(dennis)).toString()
    ...
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedDebtTokenDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedDebtTokenDeposit(dennis)).toString()
  ...
  it("redeemCollateral(): caller can redeem their entire DebtTokenToken balance", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(400, 18), extraParams: { from: alice } })
    ...
    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraDebtTokenAmount: dec(1990, 18), extraParams: { from: dennis } })
  ...
  it("redeemCollateral(): value of issued FIL == face value of redeemed DebtToken (assuming 1 DebtToken has value of $1)", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraDebtTokenAmount: dec(4990, 18), extraParams: { from: alice } })
    ...
    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraDebtTokenAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraDebtTokenAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraDebtTokenAmount: dec(1090, 18), extraParams: { from: dennis } })
    ...
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_120_debtToken, price, 0))
    ...
    assert.equal(activeFIL_1.toString(), activeFIL_0.sub(toBN(_120_debtToken).mul(mv._1e18BN).div(price)));

    // Flyn redeems 373 DebtToken
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_373_debtToken, price, 0))
    ...
    assert.equal(activeFIL_2.toString(), activeFIL_1.sub(toBN(_373_debtToken).mul(mv._1e18BN).div(price)));

    // Graham redeems 950 DebtToken
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_950_debtToken, price, 0))
    ...
    assert.equal(activeFIL_3.toString(), activeFIL_2.sub(toBN(_950_debtToken).mul(mv._1e18BN).div(price)));
  })
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    ...
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraDebtTokenAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraDebtTokenAmount: dec(40, 18), extraParams: { from: dennis } })
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero FILFee to LQTY staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LQTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
    await lqtyStaking.stake(dec(1, 18), { from: multisig })
  ...
  it("redeemCollateral(): a redemption made at zero base increases the FIL-fees-per-LQTY-staked in LQTY Staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LQTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
    await lqtyStaking.stake(dec(1, 18), { from: multisig })
    ...
    // Check LQTY Staking FIL-fees-per-LQTY-staked before is zero
    ...
    // Check LQTY Staking FIL-fees-per-LQTY-staked after is non-zero
    const F_FIL_After = await lqtyStaking.F_FIL()
    assert.isTrue(F_FIL_After.gt('0'))
  })

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero FILFee to LQTY staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LQTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
    await lqtyStaking.stake(dec(1, 18), { from: multisig })
  ...
  it("redeemCollateral(): a redemption made at a non-zero base rate increases FIL-per-LQTY-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and multisig stakes 1 LQTY
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider)
    await lqtyToken.approve(lqtyStaking.address, dec(1, 18), { from: multisig })
    await lqtyStaking.stake(dec(1, 18), { from: multisig })
  ...
  it("redeemCollateral(): a redemption sends the FIL remainder (FILDrawn - FILFee) to the redeemer", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const { netDebt: W_netDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraDebtTokenAmount: dec(10000, 18), extraParams: { from: whale } })

    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: D } })
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's FIL surplus (collateral - FIL drawn) available for the trove owner after re-opening trove", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: C } })
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { debtTokenAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraDebtTokenAmount: dec(1, 24), extraParams: { from: alice } })
    ...
    for (let i = 0; i < 2; i++) {
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      await borrowerOperations.adjustTrove(th._100pct, 0, debtTokenAmount, true, alice, alice, { from: alice, value: debtTokenAmount.mul(mv._1e18BN).div(price) })
    }
  ...
  it("getPendingDebtReward(): Returns 0 if there is no pending Debt reward", async () => {
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraDebtTokenAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraDebtTokenAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraDebtTokenAmount: totalDebt, extraParams: { from: whale } })
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraDebtTokenAmount: B_totalDebt, extraParams: { from: A } })
